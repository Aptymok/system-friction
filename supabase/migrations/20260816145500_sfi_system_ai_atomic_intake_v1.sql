-- SFI System / AI Assurance atomic intake V1
...
begin
  ...
  if v_object_exists and coalesce(v_existing_object.canonical_ref->>'hash','') <> v_object_hash then
  ...
  loop
    ...
    if found then
      if v_existing_relation.relation_type <> v_relation_type
         ...
         or coalesce(v_existing_relation.from_ref->>'id','') <> v_from_id
         or coalesce(v_existing_relation.to_ref->>'id','') <> v_to_id then
