...
for (const token of ['executeSfzAcousticRender','resolveProductionInstrument','materializeRemoteAcousticPackage','NO_ARRANGEMENT_CHANGE_WITHOUT_EXPLICIT_STEMS_OR_PERFORMANCE',"cleanupState: 'FAIL'"]) assert.ok(production.includes(token), `production_missing:${token}`);
assert.doesNotMatch(production, /instrumentRegistry\.ts|\.\/instrumentRegistry|\.\/sfzAdapter/);
for (const token of ["from('sfi_instruments')",".eq('current_execution_rights_state', 'ELIGIBLE')",".eq('quality_state', 'PRODUCTION')",'DERIVATIVE_ALLOWED','EXECUTION_ALLOWED']) assert.ok(resolver.includes(token), `resolver_missing:${token}`);
...
for (const token of ['beforeObservation = await analyzeStudioAudioObject','afterObservation = await analyzeStudioAudioObject','PROCESSING_PERMISSION_ONLY_NOT_RIGHTS_TRANSFER','rightsTransfer: false','canonicalPromotion: false']) assert.ok(studioProduction.includes(token), `studio_production_missing:${token}`);
...
console.log(JSON.stringify({ ok:true, contract:'SFI-MATERIAL-AUDIO-PRODUCTION-GATE-1.0', modes:['VOICE_MUSICALIZE','MASTER_ADJUST'], canonicalInstrumentOwner:'public.sfi_instruments', canonicalRenderer:'executeSfzAcousticRender', scope:'studio:run', rightsTransfer:false, canonicalPromotion:false, realNetworkSampleQa:true, duplicateOwners:0 }, null, 2));
