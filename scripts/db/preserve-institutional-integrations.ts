...
async function loadState(): Promise<State> {
  ...
  for (const item of parsed.members) {
    ...
    if (!registryByEmail.has(email)) throw new Error(`SFI_INSTITUTIONAL_MEMBER_NO_LONGER_REGISTERED:${email}`);
...
if (mode === 'restore') {
  ...
  for (const item of state.members) {
    ...
    const scopeRows = runPsql(`select allowed_scopes::text from public.sfi_oauth_clients where created_by=${q(item.user_id)}::uuid and status='ACTIVE' order by id;`).split(/\r?\n/).filter(Boolean);
    for (const value of scopeRows) {
      const observed = value.replace(/^\{|\}$/g,'').split(',').filter(Boolean).map((s)=>s.replace(/^"|"$/g,'')).sort();
