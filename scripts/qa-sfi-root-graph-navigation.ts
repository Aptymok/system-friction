...
check('canonical graph reads trim duplicate payload columns in both stores', canonicalGraphRuntime.includes('GRAPH_NODE_READ_FIELDS') && canonicalGraphRuntime.includes('GRAPH_EDGE_READ_FIELDS') && !canonicalGraphRuntime.includes("from('graph_nodes').select('*')") && !canonicalGraphRuntime.includes("from('graph_edges').select('*')") && continuityStore.includes('select id,node_id,node_key,label,node_type,ontology_type,origin,attributes,lineage,created_at,updated_at') && !continuityStore.includes('select *\n              from graph_nodes'));
