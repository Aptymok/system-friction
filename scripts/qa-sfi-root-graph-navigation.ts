...
check('ROOT is regenerated as a thin sovereign projection over existing owners', rootUi.includes('ROOT · SOBERANÍA INSTITUCIONAL') && rootUi.includes('Gobierna cambios. Observa el resto.') && rootUi.includes("jsonFetch('/api/root/interactive?surface=root')") && rootUi.includes("jsonFetch('/api/root/decisions'"));
...
check('ROOT UI exposes only binary sovereign decisions', rootUi.includes('ACEPTAR') && rootUi.includes('DENEGAR') && !rootUi.includes('PEDIR EVIDENCIA') && rootUi.includes('ROOT no aprueba fuentes'));
...
check('ROOT governance workspace reads governed proposals from canonical sovereign-only interactive projection', governanceUi.includes("jsonFetch('/api/root/interactive?surface=governance')") && governanceUi.includes('setProposals(arr(operationalNext.items))') && interactiveRoute.includes("proposalQueueSource: 'operationalNext.items(rootActionRequired=true)'"));
