...
check('canonical graph retries the minimal field projection only for a missing-column schema mismatch', canonicalGraphRuntime.includes('readSupabaseGraphRows') && canonicalGraphRuntime.includes('isMissingGraphColumn') && canonicalGraphRuntime.includes('GRAPH_NODE_CANONICAL_READ_FIELDS') && !canonicalGraphRuntime.includes("from('graph_nodes').select('*')") && !canonicalGraphRuntime.includes("from('graph_edges').select('*')"));
