...
const summary={checkedFiles:files.length,violations:violations.length,byRule:Object.fromEntries([...new Set(violations.map(v=>v.rule))].sort().map(rule=>[rule,violations.filter(v=>v.rule===rule).length]))};
