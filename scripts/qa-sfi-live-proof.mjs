...
async function main() {
  ...
  const loginPage = await fetchJson('/login', { headers: { accept: 'text/html' } });
  const continuityAccessPage = await fetchJson('/continuity-access', { headers: { accept: 'text/html' } });
  ...
  const rootAuth = cookie
    ...
    : {
        ...
        json: null,
        text: null,
        error: 'BLOCKED_BY_AUTH_FIXTURE: set SFI_ROOT_COOKIE_HEADER to a valid authenticated ROOT SFI session Cookie header',
      };
  ...
  const report = {
    ...
    root_authenticated: {
      ...
      required_fixture: cookie
        ? null
        : 'SFI_ROOT_COOKIE_HEADER with a valid authenticated ROOT SFI session Cookie header',
      backend_assumption: 'NONE_AUTHORITY_IS_SESSION_BOUND_NOT_PROVIDER_BOUND',
    },
  };
