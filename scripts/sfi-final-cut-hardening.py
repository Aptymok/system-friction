...
cycle = 'src/lib/sfi/universalSignalCycle.ts'
replace_exact(
    cycle,
...
risk = 'src/lib/sfi/cognitive-runtime/agents/riskAgent.ts'
replace_exact(
    risk,
...
resolver = 'src/lib/sfi/evidenceRequirementResolver.ts'
replace_exact(
    resolver,
...
qa = Path('scripts/qa-sfi-web-source-extraction.ts')
q = qa.read_text()
old = "const hydrator = fs.readFileSync(path.join(process.cwd(), 'src/lib/sfi/universalObservationHydrator.ts'), 'utf8');"
...
if q.count(old) != 1:
    raise SystemExit('qa import anchor mismatch')
q = q.replace(old, new)
q = q.replace(
...
anchor = "// RETURN evidence lineage must be explicit server-derived linkage only. Generic\n"
assertions = """// Direct material content must survive into runtime evidence, while unverified caller extraction cannot self-promote.
...
"""
if anchor not in q:
    raise SystemExit('qa assertion anchor missing')
q = q.replace(anchor, assertions + anchor)
qa.write_text(q)

# Remove every temporary repair artifact, including this script and the injected job.
for temporary in [Path('.github/workflows/sfi-final-cut-hardening.yml'), Path('.sfi-final-cut-trigger')]:
    if temporary.exists():
        temporary.unlink()

workflow = Path('.github/workflows/sfi-universal-signal.yml')
w = workflow.read_text()
start_marker = '  # SFI_FINAL_HARDENING_BEGIN\n'
end_marker = '  # SFI_FINAL_HARDENING_END\n'
start = w.find(start_marker)
end = w.find(end_marker)
if start < 0 or end < 0 or end < start:
    raise SystemExit('self-removal markers missing')
end += len(end_marker)
workflow.write_text(w[:start] + w[end:])
