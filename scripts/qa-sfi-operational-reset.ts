...
for(const table of ['profiles','system_accounts','system_roles','system_permissions','system_access_grants','system_entitlements','worldspect_snapshots'])assert.ok(PROTECTED_TABLES.includes(table),`missing_protected_table:${table}`);
