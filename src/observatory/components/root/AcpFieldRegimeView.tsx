...
type TwinState = {
  data?: {
    seed?: {
      nodeCatalog?: unknown[];
      mihmRuntimeMatrix?: { sourceState?: string; ihg?: number; nti?: number; phi?: number; regime?: string };
    };
  };
};
...
const STATE_COLOR = { observed: '#1D9E75', latent: '#4a4845', degraded: '#E8593C', resonant: '#7F77DD', stable: '#C8A951' };
...
function focusBoost(node: FieldNode, focusMode?: string) {
  if (!focusMode) return node.p;
  if (focusMode === 'gobernanza' && node.cluster === 'governance') return Math.min(1, node.p + 0.35);
  if (focusMode === 'memoria' && node.cluster === 'archive') return Math.min(1, node.p + 0.35);
  if (focusMode === 'energia' && (node.cluster === 'operational' || node.cluster === 'mihm')) return Math.min(1, node.p + 0.25);
  if (focusMode === 'contradiccion' && node.state === 'degraded') return Math.min(1, node.p + 0.35);
  if (focusMode === 'validacion' && node.cluster === 'evidence') return Math.min(1, node.p + 0.25);
  if (focusMode === 'atractores' && (node.cluster === 'sf' || node.cluster === 'mihm' || node.cluster === 'governance')) return Math.min(1, node.p + 0.25);
  return node.p * 0.78;
}
...
function makeNodes(twin: TwinState | null, width: number, height: number) {
  ...
  const nodes = source.slice(0, 180).map((node, index) => {
    ...
    const spread = 42 + ratio(seed, 2) * 72;
...
function draw(ctx: CanvasRenderingContext2D, nodes: FieldNode[], width: number, height: number, tick: number, focusMode?: string) {
  ...
  for (let i = 0; i < 520; i += 1) {
    ...
    ctx.fillRect(hash(`x-${i}-${Math.floor(tick / 9)}`) % width, hash(`y-${i}-${Math.floor(tick / 13)}`) % height, 1, 1);
  }
  ctx.globalAlpha = 1;
  ...
  Object.values(CLUSTERS).forEach((cluster, index) => {
    ...
    ctx.beginPath();
    ctx.arc(cx, cy, 78 + Math.sin(tick * 0.012 + index) * 8, 0, Math.PI * 2);
    ctx.fillStyle = `${cluster.color}10`;
    ctx.fill();
    ctx.strokeStyle = `${cluster.color}22`;
    ctx.setLineDash([3, 7]);
    ctx.stroke();
    ctx.setLineDash([]);
  });
  ...
  nodes.forEach((node) => {
    const boosted = focusBoost(node, focusMode);
    node.links.slice(0, 2).forEach((targetIndex) => {
      ...
      ctx.strokeStyle = `rgba(200,169,81,${Math.max(0.025, (1 - distance / 230) * 0.18 + boosted * 0.08)})`;
      ctx.lineWidth = 0.28 + boosted * 0.55;
      ctx.stroke();
    });
  });
  ...
  nodes.forEach((node, index) => {
    ...
    node.vx += (dx / d) * (0.00042 + boosted * 0.00022);
    node.vy += (dy / d) * (0.00042 + boosted * 0.00022);
    node.vx *= 0.992;
    node.vy *= 0.992;
    node.x += node.vx;
    node.y += node.vy;
    ...
    const r = node.r + boosted * 1.6 + (node.state === 'resonant' || node.state === 'degraded' ? pulse * 1.35 : 0);
    ...
    ctx.globalAlpha = 0.35 + boosted * 0.55;
    ctx.fill();
    ctx.globalAlpha = 1;
    if (index < 5 || boosted > 0.82) {
...
export function AcpFieldRegimeView({ twin, focusMode }: { twin: TwinState | null; focusMode?: string }) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const nodesRef = useRef<FieldNode[]>([]);
  const frameRef = useRef<number | null>(null);
  const tickRef = useRef(0);
  const focusRef = useRef<string | undefined>(focusMode);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      tickRef.current += 1;
      draw(ctx, nodesRef.current, size.width, size.height, tickRef.current, focusRef.current);
  ...
  return (
    <section className="relative h-full min-h-[calc(100vh-72px)] bg-[#080808]">
      <canvas ref={canvasRef} className="block h-full w-full" />
      <div className="pointer-events-none absolute bottom-3 right-4 border border-[#1e1c17] bg-[#060605]/75 px-3 py-2 font-mono text-[8px] uppercase tracking-[0.14em] text-[#4a4845]">
        {nodes.length} nodos · {observed} observados · {degraded} degradados · ruido activo
      </div>
    </section>
  );
}
