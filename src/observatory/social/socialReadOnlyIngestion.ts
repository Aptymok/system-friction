// src/observatory/social/socialReadOnlyIngestion.ts
...
function scoreFromEngagement(
  engagement: Record<string, number>,
) {
  const values =
    Object.values(engagement)
...
export function assertSocialWriteDisabled() {
  return {
    canPublish: false,
    reason:
      'OAuth write no habilitado.',
  }
}

export function normalizeSocialMetrics(
  provider: SocialProvider,
  rawPayload: Record<string, unknown>,
): SocialMetricSnapshot {
  const engagement =
    numericRecord(
      (rawPayload.engagement ||
        rawPayload.public_metrics ||
  ...
  return {
    ...
    rawPayload,
    sourceDescriptor:
      createObservationSourceDescriptor({
        sourceState:
          'SOCIAL_RETURN',
        confidence:
          'limited',
...
export async function getConnectedSocialSources(
  ctx: ServerContext,
) {
  const {
    data: tokens,
...
async function upsertTelemetrySource(
  ctx: ServerContext,
  provider: SocialProvider,
  scope: string[],
  lastSyncAt: string,
...
async function fetchLinkedInPosts(
  ...
  const response =
    await fetch(
      `https://api.linkedin.com/v2/ugcPosts?q=authors&authors=List(${encodeURIComponent(author)})`,
      {
        headers: {
          Authorization:
            `Bearer ${token.access_token}`,
...
async function persistSocialMetricSnapshot(
  ctx: ServerContext,
  snapshot: SocialMetricSnapshot,
) {
  const engagementHash =
    hashEngagement(
      snapshot.engagement,
    )
  ...
  const {
    ...
    .eq(
      'raw_payload->>captureMode',
      'oauth_read_only',
    )
    .eq(
      'raw_payload->>engagementHash',
      engagementHash,
    )
    .limit(1)
    .maybeSingle()
  ...
  return ctx.service
    ...
    .insert({
      ...
      post_id:
        snapshot.postId,
      resonance_score:
        snapshot.resonanceScore,
      engagement:
        snapshot.engagement,
      comments_summary:
        snapshot.commentsSummary,
      raw_payload: {
        ...snapshot.rawPayload,
        sourceState:
          'SOCIAL_RETURN',
...
async function persistExternalSignal(
  ctx: ServerContext,
  telemetrySourceId: string | null,
  post: SocialIngestedPost,
) {
  const {
    data: existing,
  } = await ctx.service
...
export async function ingestSocialMetrics(
  ...
  try {
    ...
    for (const post of posts) {
      await persistExternalSignal(
        ctx,
        telemetrySource?.id || null,
        post,
      )
      ...
      await persistSocialMetricSnapshot(
        ctx,
        snapshot,
      )
