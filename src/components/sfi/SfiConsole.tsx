...
function LegacySceneConsole({scene}:{scene:SceneKey}){
 const spec=SCENES[scene],auth=useAuthState(); const [clock,setClock]=useState(''); const [live,setLive]=useState<any>(null); const [proposals,setProposals]=useState<Proposal[]>([]); const [proposalError,setProposalError]=useState<string|null>(null); const [rootPresenceReady,setRootPresenceReady]=useState(scene!=='root'); const [selected,setSelected]=useState<Proposal|null>(null); const [open,setOpen]=useState(false);
 ...
 useEffect(()=>{if(auth.status!=='authenticated'||!rootPresenceReady)return;let stop=false;const pull=async()=>{try{const r=await fetch('/api/acp/proposals',{cache:'no-store'});const j=await r.json().catch(()=>null);if(stop)return;if(r.ok&&j?.ok){setProposals(j.data?.proposals||[]);setProposalError(null)}else{setProposals([]);setProposalError(`${r.status}: ${j?.error||'proposal_source_failed'}`)}}catch(error){if(!stop){setProposals([]);setProposalError(error instanceof Error?error.message:String(error))}}};void pull();const t=setInterval(pull,15000);return()=>{stop=true;clearInterval(t)}},[auth.status,rootPresenceReady]);
 const liveCount=useMemo(()=>live?.data?.tables?.length??live?.data?.proposals?.length??Object.keys(live?.data||{}).length,[live]);
 const decide=async(kind:'approve'|'reject')=>{if(!selected)return;await fetch(`/api/acp/proposals/${selected.id}/${kind}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({note:'Decisión ROOT desde SFI Live Interface'})});setSelected(null);const r=await fetch('/api/acp/proposals',{cache:'no-store'});const j=await r.json();if(j?.ok){setProposals(j.data.proposals||[]);setProposalError(null)}else setProposalError(`${r.status}: ${j?.error||'proposal_source_failed'}`)};
 return <main className="sfi"><div className={`scene cinematic scene-${scene}`} style={{backgroundImage:`linear-gradient(90deg,rgba(5,5,4,.66),rgba(5,5,4,.06) 55%,rgba(5,5,4,.72)),url(${spec.image})`}}><Instrument scene={scene}/><div className="scan"/><div className="orbital"/><div className="pulse p1"/><div className="pulse p2"/><div className="pulse p3"/>
   ...
   <section className="twin"><div className="twinHead"><span>COGNITIVE TWIN</span><b>{proposals.filter(p=>['proposed','waiting_evidence','needs_evidence'].includes(p.status||'')).length} por decidir</b></div><p>Propone cambios; la autoridad permanece en ROOT. Sin jerga operativa.</p><div className="proposalList">{proposals.slice(0,5).map(p=><button key={p.id} onClick={()=>setSelected(p)}><strong>{p.title||p.proposalType||'Propuesta'}</strong><span>{p.status||'propuesta'} · riesgo {p.risk_level||'no indicado'}</span></button>)}{proposalError&&<em>Fuente de propuestas DEGRADED · {proposalError}</em>}{!proposalError&&!proposals.length&&<em>No hay propuestas pendientes visibles con esta sesión.</em>}</div></section>
