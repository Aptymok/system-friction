...
export function SfiGovernanceWorkspace({enabled}:{enabled:boolean}){
  ...
  const loadBase=useCallback(async()=>{if(!enabled)return;try{
    const [rt,root,ci,wb]=await Promise.all([
      jsonFetch('/api/root/cognitive-runtime'),
      jsonFetch('/api/root/console'),
      jsonFetch('/api/cases'),
      jsonFetch('/api/root/workboard'),
    ]);
  ...
  const loadDossier=useCallback(async(id:string)=>{if(!enabled||!id)return;try{const result=await jsonFetch(`/api/root/cognitive-runtime/records?agentId=${encodeURIComponent(id)}&limit=100`);setDossier(result);setSelectedExecutionId(current=>current&&arr(result.history).some(item=>item.executionId===current)?current:(result.state?.latestExecutionId??arr(result.history)[0]?.executionId??null));setError(null)}catch(cause){setError(cause instanceof Error?cause.message:String(cause))}},[enabled]);
  ...
  const targetOptions=useMemo<AgentTarget[]>(()=>{
    ...
    const allowed=new Set<string>(arr(contract?.allowedTargetKinds).map(String));
  ...
  },[projects,cases,cycles,evidenceEntries,evidenceNodes,contract,targetQuery]);

  useEffect(()=>{setSelectedTargets(current=>current.filter(item=>!contract?.allowedTargetKinds||contract.allowedTargetKinds.includes(item.kind)).slice(0,contract?.maxTargets??8));setDirection(contract?.allowedDirections?.[0]??'');setParameters({});setAgentResult(null)},[agentId,contract?.version]);
  ...
  const missingRequiredParameters=arr(contract?.requiredParameters).map(String).filter(key=>!parameters[key]?.trim());
  ...
  const runAgent=async()=>{if(!canExecute||!contract)return;setBusy('agent');setAgentResult(null);try{
    ...
    const anchorTarget=selectedTargets.find(item=>arr(contract.allowedAnchorKinds).map(String).includes(item.kind));
  ...
  const parameterKeys=[...new Set([...arr(contract?.requiredParameters).map(String),...arr(contract?.optionalParameters).map(String)])];
  ...
  return <div className="sfiGovernanceLayout">
    ...
    <section className="sfiGovernancePanel operatorPanel"><header><span>DOSSIER OPERACIONAL · {selectedAgent?.name??agentId}</span><button className="heartbeat" disabled={busy==='heartbeat'} onClick={()=>void heartbeat()}>{busy==='heartbeat'?'Ejecutando…':'Ejecutar heartbeat ahora'}</button></header>
      <div className="sfiOperatorForm">
        ...
        <div className="sfiMetaGrid"><span>LEE: {arr(dossier?.passport?.reads).map(item=>txt(item.memory??item)).join(', ')||'—'}</span><span>ESCRIBE: {arr(dossier?.passport?.writes).map(item=>txt(item.memory??item)).join(', ')||'—'}</span><span>EMITE: {arr(dossier?.passport?.emits).map(item=>txt(item)).join(', ')||'—'}</span><span>Perfil: {contract?.governanceProfile??'—'}</span></div>
        ...
        <h3>EJECUTAR SEGÚN CONTRATO</h3>
        <p>Objetivos requeridos: {minTargets}–{maxTargets}. Tipos permitidos: {arr(contract?.allowedTargetKinds).map(String).join(', ')||'—'}.</p>
        ...
        {arr(contract?.allowedDirections).length>0&&<label>Dirección<select value={direction} onChange={event=>setDirection(event.target.value)}>{arr(contract.allowedDirections).map(item=><option key={String(item)} value={String(item)}>{String(item)}</option>)}</select></label>}
        ...
        {parameterKeys.length>0&&<div><h3>Parámetros del agente</h3>{parameterKeys.map(key=><label key={key}>{key}{arr(contract?.requiredParameters).map(String).includes(key)?' · requerido':' · opcional'}<input value={parameters[key]??''} onChange={event=>setParameters(current=>({...current,[key]:event.target.value}))}/></label>)}</div>}

        <h3>Preflight contextual</h3>
        <div className="sfiMetaGrid"><label>Sujeto<select value={subjectType} onChange={event=>setSubjectType(event.target.value)}>{['NOT_DECLARED','SYSTEM','ORGANIZATION','PERSON','GROUP','MIXED'].map(item=><option key={item}>{item}</option>)}</select></label><label>Jurisdicción<input value={jurisdiction} onChange={event=>setJurisdiction(event.target.value)} placeholder="MX, EU, organización interna…"/></label><label>Datos personales<select value={personalData} onChange={event=>setPersonalData(event.target.value as TriState)}>{['NOT_DECLARED','YES','NO'].map(item=><option key={item}>{item}</option>)}</select></label><label>Datos sensibles<select value={sensitiveData} onChange={event=>setSensitiveData(event.target.value as TriState)}>{['NOT_DECLARED','YES','NO'].map(item=><option key={item}>{item}</option>)}</select></label><label>Decisión sobre personas<select value={personDecision} onChange={event=>setPersonDecision(event.target.value as TriState)}>{['NOT_DECLARED','YES','NO'].map(item=><option key={item}>{item}</option>)}</select></label><label>Base declarada<input value={purposeBasis} onChange={event=>setPurposeBasis(event.target.value)} placeholder="Base jurídica/organizacional declarada; no se presume"/></label></div>
        <button className="primaryAction" disabled={!canExecute} onClick={()=>void runAgent()}>{busy==='agent'?'Ejecutando agente…':'EJECUTAR CONTRATO'}</button>
        {missingRequiredParameters.length>0&&<p>Faltan parámetros requeridos: {missingRequiredParameters.join(', ')}.</p>}
        ...
        {dossier?.historyRead&&<p>{arr(dossier.historyRead.warnings).map(String).join(' · ')}</p>}
      </div>
    </section>
