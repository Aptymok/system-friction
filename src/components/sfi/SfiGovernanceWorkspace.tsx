...
export function SfiGovernanceWorkspace({enabled}:{enabled:boolean}){
  ...
  return <div className="sfiGovernanceLayout">
    ...
    <section className="sfiGovernancePanel operatorPanel"><header><span>RUNTIME / AGENTES · {selectedAgent?.name??'Selecciona un agente'}</span><button className="heartbeat" disabled={busy==='heartbeat'||!agentId} onClick={()=>void heartbeat()}>{busy==='heartbeat'?'Ejecutando…':'Ejecutar heartbeat ahora'}</button></header><div className="sfiOperatorForm">
      {!agentId&&<div className="sfiAgentResult"><h3>HIDRATACIÓN DIFERIDA</h3><p>Passports y contratos están disponibles sin cargar Cases, Evidence, Graph, ciclos ni historial. Selecciona un agente para leer únicamente los targets y registros necesarios para operarlo.</p></div>}
      {agentId&&<><div className="sfiMetaGrid"><span>Infraestructura: <Status value={state?.infrastructure}/></span><span>Trabajo: <Status value={state?.work}/></span><span>Epistemología: <Status value={state?.epistemic}/></span><span>Autoridad: <Status value={state?.authority}/></span><span>Última ejecución: {date(state?.latestExecutionAt)}</span><span>Última inferencia: {date(state?.latestInferenceAt)}</span><span>Interacción genérica: {state?.latestInteractionObservation==='OBSERVED'?date(state?.latestInteractionAt):'NO OBSERVADA'}</span><span>Contrato: {contract?.version??'—'}</span></div>
