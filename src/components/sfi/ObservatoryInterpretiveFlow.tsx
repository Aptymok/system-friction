...
export function ObservatoryInterpretiveFlow(){
  const {text}=useSfiLanguage();
  const [world,setWorld]=useState<Row|null>(null);
  ...
  useEffect(()=>{
    ...
    const pull=async()=>{
      try{
        ...
        if(!stop)setWorld(data);
      }catch{if(!stop)setWorld(null)}
    };
    void pull();
  ...
  return <section className="obsInterpretiveFlow" aria-label={text('Lectura interpretativa del campo','Interpretive field reading')}>
    <div className="obsInterpretiveIntro">
      ...
      <div className="obsFlowStats"><span>{nodes.length} {text('observaciones','observations')}</span><span>{sourceFamilies.length} {text('familias de fuente','source families')}</span><span>{hypotheses.length} {text('hipótesis trazables','traceable hypotheses')}</span></div>
    </div>
