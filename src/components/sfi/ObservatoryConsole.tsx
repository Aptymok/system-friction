...
export function ObservatoryConsole(){
  ...
  const narrative=availability.world!=='AVAILABLE'
    ? ownedText(`Authoritative field read: ${availability.world}. Counts remain non-numeric until a successful read.`,`Authoritative field read: ${availability.world}. Counts remain non-numeric until a successful read.`)
    : selectedHypothesis
      ? ownedText(`The selected hypothesis is an inference, not a fact: ${selectedHypothesis.statement??'no statement'}. Its trace uses ${selectedEvidenceIds.size} source records, affects ${arr(selectedHypothesis.aiInference?.affectedSystems).length} systems and preserves explicit contradiction signals.`,`The selected hypothesis is an inference, not a fact: ${selectedHypothesis.statement??'no statement'}. Its trace uses ${selectedEvidenceIds.size} source records, affects ${arr(selectedHypothesis.aiInference?.affectedSystems).length} systems, and preserves explicit contradiction signals.`)
      : ownedText(`The field contains ${nodes.length} visible observations and ${filteredHypotheses.length} traceable hypotheses under the current filters.`,`The field contains ${nodes.length} visible observations and ${filteredHypotheses.length} traceable hypotheses under the current filters.`);
  ...
  return <><main className="obsShell" data-world-availability={availability.world} data-state-availability={availability.state} data-timeline-availability={availability.timeline}><section className={`obsScene lens-${lens}`}><div className="starfield"/><div className="deepSpace"/>
    ...
    <header className="obsTop"><div className="obsBrand"><strong>SFI</strong><span>{ui('FIELD · SYSTEM FRICTION INSTITUTE')}</span><small>{ui('OBSERVATORIO MUNDIAL EN VIVO')}</small></div>
      <nav>{(['field','hypotheses','trajectory','sources'] as Lens[]).map(k=><button key={k} className={lens===k?'active':''} onClick={()=>{setLens(k);setSatelliteOpen(true)}}>{k==='hypotheses'?ownedText('HYPOTHESES','HYPOTHESES'):k==='trajectory'?ownedText('TRAYECTORIA','TRAJECTORY'):k==='sources'?ownedText('FUENTES','SOURCES'):ownedText('CAMPO','FIELD')}</button>)}<button onClick={()=>void pull(true)} disabled={refreshing}>{refreshing?ownedText('LEYENDO…','READING…'):ownedText('ACTUALIZAR','REFRESH')}</button><Link href="/history">{ui('ORIGEN → AHORA')}</Link>{auth.status==='authenticated'&&<Link href="/cases">{ui('CASOS')}</Link>}</nav>
      <div className="obsIdentity"><b>{auth.identity?.alias||'PUBLIC'}</b><span>{lastReadAt?`${ownedText('READ','READ')} ${lastReadAt.slice(11,19)} UTC`:auth.identity?.role||auth.status}</span></div><SessionControls className="obsSessionControls"/></header>
    ...
    <aside className="hud hudLeft"><section><small>SFI-OBS-LIVE</small><h3>{ownedText('CAMPO VIVO','LIVE FIELD')}</h3><p className="good">● {clock.slice(11,19)} UTC</p><dl><dt>{ui('OBSERVACIONES')}</dt><dd data-availability={availability.world}>{worldMetric(nodes.length)}</dd><dt>{ui('FUENTES ACTIVAS')}</dt><dd data-availability={availability.world}>{worldMetric(sourceIds.length)}</dd><dt>{ui('HYPOTHESES')}</dt><dd data-availability={availability.world}>{worldMetric(filteredHypotheses.length)}</dd><dt>{ui('EN RETORNO')}</dt><dd data-availability={availability.world}>{worldMetric(openHypotheses)}</dd></dl><button onClick={()=>setSatelliteOpen(true)}>{ui('OPEN SATELLITE')}</button></section>
      <section><small>{ownedText('DERIVED METRICS','DERIVED METRICS')}</small><dl><dt>Fₛ</dt><dd data-availability={availability.world}>{avgFs==null?'—':avgFs.toFixed(3)}</dd><dt>NTI</dt><dd data-availability={availability.world}>{avgNti==null?'—':avgNti.toFixed(3)}</dd><dt>Φ</dt><dd data-availability={availability.world}>{avgPhi==null?'—':avgPhi.toFixed(3)}</dd></dl><p style={{fontSize:11,opacity:.62,lineHeight:1.5}}>{ownedText('The numbers describe observed/derived structure. Meaning, mechanism and consequences are shown only as traceable hypotheses.','Numbers describe observed/derived structure. Meaning, mechanism and consequences are shown only as traceable hypotheses.')}</p></section>
    </aside>
    ...
    {satelliteOpen&&<aside style={{...panel,right:18,top:104,bottom:142,width:'min(470px,42vw)',padding:16,overflowY:'auto',scrollbarWidth:'none'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10}}><div><div style={micro}>SFI SATELLITE · {lens.toUpperCase()}</div><h2 style={{fontSize:18,margin:'5px 0 0'}}>{lens==='field'?ownedText('LECTURA DEL CAMPO','FIELD READING'):lens==='hypotheses'?ownedText('GRAFO DE HYPOTHESES','HYPOTHESIS GRAPH'):lens==='trajectory'?ownedText('TRAYECTORIA Y RETORNO','TRAJECTORY & RETURN'):ownedText('FUENTES VIVAS','LIVE SOURCES')}</h2></div><button onClick={()=>setSatelliteOpen(false)} style={{...selectStyle,padding:'6px 9px'}}>×</button></div>
      <p style={{fontSize:12,lineHeight:1.6,opacity:.74}} data-availability={availability.world}>{narrative}</p>

      {lens==='field'&&<><div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,margin:'12px 0'}}>{[[ownedText('Fuentes','Sources'),worldMetric(sourceIds.length)],[ownedText('Open hypotheses','Open hypotheses'),worldMetric(openHypotheses)],[ownedText('Contrastadas','Contrasted'),worldMetric(outcomeCount)],[ownedText('Aprendizajes','Learning'),worldMetric(learningCount)],['WSI',num(obs?.data?.worldspect?.wsi??frame?.wsi)?.toFixed(3)??'—'],['NTI src',num(obs?.data?.worldspect?.nti??frame?.nti)?.toFixed(3)??'—']].map(([a,b],index)=><div key={String(a)} data-availability={index<4?availability.world:undefined} style={{padding:10,border:'1px solid rgba(214,180,120,.12)',borderRadius:9}}><div style={micro}>{a}</div><b style={{fontSize:18}}>{b}</b></div>)}</div>
        <div style={micro}>{ownedText('HYPOTHESES ACTIVAS','ACTIVE HYPOTHESES')}</div>{availability.world!=='AVAILABLE'?<p data-availability={availability.world} style={{fontSize:11,opacity:.72}}>{availability.world}</p>:filteredHypotheses.slice(0,8).map(h=><button key={String(h.id)} onClick={()=>{setSelectedHypothesisId(String(h.id));setLens('hypotheses')}} style={{display:'block',width:'100%',textAlign:'left',marginTop:7,padding:10,borderRadius:9,border:String(h.id)===String(selectedHypothesis?.id)?'1px solid rgba(214,180,120,.5)':'1px solid rgba(214,180,120,.12)',background:'rgba(255,255,255,.018)',color:'inherit'}}><b>{short(h.statement,120)||'Hypothesis'}</b><div style={{...micro,marginTop:5}}>{h.aiInference?.relationClass||'UNKNOWN'} · {pct(num(h.current_confidence))} · {h.status}</div></button>)}</>}

      {lens==='hypotheses'&&<>{availability.world!=='AVAILABLE'?<p data-availability={availability.world} style={{fontSize:11,opacity:.72}}>{ownedText('The authoritative hypothesis read is unavailable.','The authoritative hypothesis read is unavailable.')} · {availability.world}</p>:selectedHypothesis?<div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginTop:14}}><section><div style={micro}>{ownedText('EXPECTED SIGNALS','EXPECTED SIGNALS')}</div>{arr(selectedHypothesis.expected_signals).map(String).map(v=><div key={v} style={{fontSize:11,padding:'4px 0'}}>+ {v}</div>)}</section><section><div style={micro}>{ownedText('CONTRADICCIONES','CONTRADICTIONS')}</div>{arr(selectedHypothesis.contradiction_signals).map(String).map(v=><div key={v} style={{fontSize:11,padding:'4px 0'}}>− {v}</div>)}</section></div>
        <div style={{...micro,marginTop:14}}>{ownedText('HYPOTHESES RIVALES','RIVAL HYPOTHESES')}</div>{arr(selectedHypothesis.aiInference?.rivalHypotheses).map(String).map(v=><div key={v} style={{fontSize:11,padding:'4px 0'}}>{v}</div>)}
