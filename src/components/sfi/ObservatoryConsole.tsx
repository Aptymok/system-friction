...
function nodeNarrative(node:WorldNode,neighbors:WorldNode[],language:'es'|'en'){
  ...
  if(language==='es'){
    ...
    if(friction!=null)parts.push(`Systemic friction is ${friction.toFixed(3)}${density!=null?`; interaction density is ${density.toFixed(3)}`:''}${gradient!=null?`; gradient is ${gradient.toFixed(3)}`:''}${coherence!=null?`; coherence is ${coherence.toFixed(3)}`:''}.`);
...
export function ObservatoryConsole(){
  ...
  return <main className="obsShell"><section className={`obsScene lens-${lens}`}><div className="starfield"/><div className="deepSpace"/>
    ...
    {satelliteOpen&&<div className="satelliteHubPanel"><button className="close" onClick={()=>setSatelliteOpen(false)}>×</button><small>SFI-OBS-01 · {ownedText('HUBS VIVOS','LIVE HUBS')}</small><div className="hubRail">{hubs.map(([id,label,meta])=><button key={id} className={satelliteHub===id?'active':''} onClick={()=>setSatelliteHub(id)}><b>{label}</b><span>{meta}</span></button>)}</div><div className="hubBody">{satelliteHub==='daily'&&<><h2>{ui('LECTURA DIARIA')}</h2><p>{ownedText(`WSV ${wsi==null?'n/d':wsi.toFixed(3)} y NTI ${nti==null?'n/d':nti.toFixed(3)}. El campo contiene ${nodes.length} observaciones georreferenciadas, ${hypotheses.length} hipótesis y ${outcomes.length} contrastes.`,`WSV ${wsi==null?'n/d':wsi.toFixed(3)} and NTI ${nti==null?'n/d':nti.toFixed(3)}. The field contains ${nodes.length} georeferenced observations, ${hypotheses.length} hypotheses and ${outcomes.length} contrasts.`)}</p></>}{satelliteHub==='world10d'&&<><h2>{ui('10 DIMENSIONES')}</h2><div className="dimensionGrid">{(frame?.vectors||vectors).slice(0,10).map((v:any)=><div key={String(v.id||v.label)}><span>{String(v.label||v.id)}</span><b>{num(v.value)==null?'MISSING':num(v.value)!.toFixed(3)}</b></div>)}</div></>}{satelliteHub==='hypotheses'&&<><h2>{ownedText('HIPÓTESIS ABIERTAS','OPEN HYPOTHESES')}</h2><div className="hubList">{hypotheses.slice(0,8).map(h=><button key={h.id} onClick={()=>{setLens('time');setSatelliteOpen(false)}}><b>{h.statement||h.phenomenon_key||h.id}</b><span>{h.status||'OPEN'} · conf. {num(h.current_confidence)?.toFixed(2)??'n/d'}</span></button>)}</div></>}{satelliteHub==='learning'&&<><h2>{ownedText('APRENDIZAJE / RETORNO','LEARNING / RETURN')}</h2><div className="hubList">{learning.slice(0,8).map((l:any)=><div key={String(l.id)}><b>{String(l.hypothesis_id||'learning')}</b><span>{`confianza ${num(l.confidence_before)?.toFixed(2)??'n/d'} → ${num(l.confidence_after)?.toFixed(2)??'n/d'}`}</span></div>)}</div></>}</div></div>}
    {selected&&<div className="nodeTimelinePanel"><button className="close" onClick={()=>setSelectedId(null)}>×</button><small>{ownedText('HISTORIA TEMPORAL · NODO','TIME HISTORY · NODE')}</small><h2>{selected.title}</h2><div className="nodeTimelineRows"><div><time>{selected.observedAt?.slice(0,16).replace('T',' ')}</time><p>Observación registrada por {selected.publisher}. {selected.summary||''}</p></div>{selectedHypotheses.map(h=><div key={h.id}><time>{String(h.cutoff_at||'').slice(0,16).replace('T',' ')}</time><p>Hipótesis: {h.statement||h.phenomenon_key}. Estado {h.status}.</p></div>)}{selectedOutcomes.map((o:any)=><div key={String(o.id)}><time>{String(o.evaluated_at||'').slice(0,16).replace('T',' ')}</time><p>Contraste: {String(o.classification||'')} — {String(o.observed_outcome||'')}</p></div>)}</div></div>}
