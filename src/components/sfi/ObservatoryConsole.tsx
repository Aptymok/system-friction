...
function nodeNarrative(node:WorldNode,neighbors:WorldNode[]){const r=node.reading||{};const friction=num(r.systemic_friction),density=num(r.interaction_density),coherence=num(r.systemic_coherence),gradient=num(r.friction_gradient);const tension=compact(r.tension),trajectory=compact(r.trajectory);const systems=node.affectedSystems.slice(0,3).join(', ');const parts=[`Se observa ${node.title}.`];if(systems)parts.push(`El evento toca ${systems}.`);if(friction!=null)parts.push(`La fricción es ${frictionBand(friction)} (${friction.toFixed(3)})${density!=null?`; la densidad de interacción es ${density.toFixed(3)}`:''}${gradient!=null?`; el gradiente es ${gradient.toFixed(3)}`:''}${coherence!=null?`; la coherencia es ${coherence.toFixed(3)}`:''}.`);if(tension)parts.push(`La tensión se concentra en ${tension}.`);if(trajectory)parts.push(`La trayectoria registrada apunta a ${trajectory}.`);if(neighbors.length)parts.push(`Los nodos más próximos por estructura/campo son ${neighbors.slice(0,3).map(n=>n.title).join('; ')}.`);return parts.join(' ')}
