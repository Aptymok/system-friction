  ...
  return <main className="sfi-operate">
    ...
    <>
      <section className="cycle-identity">
        ...
        <button className="quiet" onClick={()=>setActive(null)}>NUEVO CICLO</button>
      </section>
      ...
      <section className="operate-columns">
        ...
        <article className="operate-card"><span>04 · INTEGRACIÓN</span><h3>Ciclo institucional</h3><p>Sincroniza el estado de SFI con el Twin, ejecuta la topología cognitiva y vuelve a sincronizar. Si falta trazabilidad ya no puede aparecer como cerrado.</p><button onClick={()=>void runInstitutional()} disabled={busy==='cycle'}>{busy==='cycle'?'EJECUTANDO…':'EJECUTAR CICLO SFI'}</button></article>
      </section>

      <section className="trace-panel"><div><span>BITÁCORA DEL CICLO</span><h3>Qué existe de verdad</h3></div><div className="trace-grid"><Trace label="Evidencia" values={evidenceRefs}/><Trace label="Studio" values={arrays(active.studio_object_refs)}/><Trace label="Laboratorio" values={arrays(active.method_lab_refs)}/><Trace label="Field" values={active.field_case_ref?[active.field_case_ref]:[]}/><Trace label="Retornos" values={arrays(active.return_refs)}/><Trace label="Twin" values={arrays(active.cognitive_twin_refs)}/><Trace label="Gobernanza" values={arrays(active.governance_refs)}/></div></section>
    </>}

    {cycles.length?<section className="recent-cycles"><header><span>HISTORIAL</span><h3>Ciclos recientes</h3></header>{cycles.slice(0,8).map(cycle=><button key={cycle.id} onClick={()=>setActive(cycle)} data-active={active?.id===cycle.id}><b>{cycle.title}</b><span>{cycle.status} · {cycle.cycle_code}</span></button>)}</section>:null}
