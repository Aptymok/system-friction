...
export function RootSovereignConsole({ initialState, accessMode = 'sovereign', actorLabel = 'ROOT' }: {
  ...
  function requestAction(action: RootActionRequest) {
    if (readOnly) {
      setEvents((current) => [{ id: `observer-block-${Date.now()}`, at: new Date().toISOString(), label: action.label, status: 'blocked', detail: 'OBSERVER puede leer ROOT y aportar evidencia, pero no ejecutar acciones soberanas.', auditId: null }, ...current].slice(0, 30));
      return;
    }
    setPending(action);
    setConfirmed(false);
