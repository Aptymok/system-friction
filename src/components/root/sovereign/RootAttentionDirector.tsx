...
function clickRootControl(label: string) {
  const buttons = Array.from(document.querySelectorAll<HTMLButtonElement>('.rgo-root button'));
  const target = buttons.find((button) => button.textContent?.trim().toUpperCase().includes(label.toUpperCase()));
  target?.click();
  return Boolean(target);
}

function navigate(detail: { mode: 'observatory' | 'governance' | 'conversion'; lens?: string }) {
  const modeLabel = detail.mode === 'conversion' ? 'CONVERSIÓN' : detail.mode === 'governance' ? 'GOBERNANZA' : 'OBSERVATORIO';
  clickRootControl(modeLabel);
  window.setTimeout(() => {
    if (detail.lens) clickRootControl(detail.lens);
    document.querySelector('.rgo-root')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, 40);
}
  ...
  useEffect(() => {
    ...
    async function inspect(surface: SurfaceState): Promise<SurfaceState> {
      try {
        ...
        if (!response.ok) return { ...surface, status: 'attention', detail: `HTTP ${response.status}. La ruta no respondió correctamente.` };
  ...
  return (
    <section className="rad-root" aria-label="Director de atención ROOT">
      <header className="rad-header">
        <div><span>PROJECT EXECUTION MANAGER · VENTANA INICIAL</span><h1>Qué requiere tu atención ahora</h1><p>{directorMessage}</p></div>
        <button type="button" onClick={() => navigate({ mode: 'governance' })}>ABRIR DECISIONES</button>
      </header>

      <div className="rad-priorities">
        <button type="button" onClick={() => navigate({ mode: 'conversion' })}><span>CASOS NUEVOS PROPUESTOS</span><strong>{proposedCases}</strong><p>Empresas, señales y propuestas que todavía necesitan selección, metodología o contacto.</p><b>IR A CASOS →</b></button>
        <button type="button" data-urgent={staleEvidence > 0} onClick={() => navigate({ mode: 'observatory', lens: 'ATRACTORES' })}><span>EVIDENCIA PENDIENTE</span><strong>{pendingEvidence}</strong><p>{staleEvidence ? `${staleEvidence} solicitudes llevan al menos 48 horas sin cierre.` : 'Solicitudes abiertas de evidencia y relaciones que todavía no sostienen una decisión.'}</p><b>IR A ATRACTORES →</b></button>
        <button type="button" data-urgent={degradedSurfaces.length > 0} onClick={() => document.getElementById('rad-site-health')?.scrollIntoView({ behavior: 'smooth' })}><span>SALUD DEL SITIO</span><strong>{degradedSurfaces.length ? `${degradedSurfaces.length} ATTENTION` : 'OK'}</strong><p>Studio, Field, Observatory y la capa de conversión comprobados desde la sesión ROOT.</p><b>VER SUPERFICIES →</b></button>
        <button type="button" data-urgent={inactiveAgents.length > 0} onClick={() => navigate({ mode: 'observatory', lens: 'AGENTES' })}><span>AGENTES A REVISAR</span><strong>{inactiveAgents.length}</strong><p>Errores, estados degradados o agentes sin ejecución registrada durante más de 72 horas.</p><b>IR A AGENTES →</b></button>
      </div>
      ...
      <div className="rad-detail-grid">
        <section id="rad-site-health" className="rad-health">
          ...
          {surfaces.map((surface) => <a key={surface.id} href={surface.path.startsWith('/api/') ? '#' : surface.path} onClick={(event) => { if (surface.path.startsWith('/api/')) event.preventDefault(); }} data-status={surface.status}><i aria-hidden="true" /><div><strong>{surface.label}</strong><p>{surface.detail}</p></div><b>{surface.status === 'checking' ? 'CHECKING' : surface.status.toUpperCase()}</b></a>)}
        </section>
