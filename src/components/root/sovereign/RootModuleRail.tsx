...
type InternalModule = { id: RootViewId; label: string; key: string };
type ExternalModule = { id: 'commercial'; label: string; key: string; href: string };
...
const MODULES: Array<InternalModule | ExternalModule> = [
  ...
  { id: 'commercial', label: 'COMMERCIAL INTELLIGENCE', key: '09', href: '/root/commercial' },
...
export function RootModuleRail({ active, onChange }: { active: RootViewId; onChange: (view: RootViewId) => void }) {
  return <nav className="rs-rail" aria-label="ROOT modules">
    {MODULES.map((module) => {
      const isActive = module.id === active;
      return <button key={module.id} type="button" className={isActive ? 'active' : ''} aria-current={isActive ? 'page' : undefined} onClick={() => {
        if ('href' in module) { window.location.href = module.href; return; }
        onChange(module.id);
      }}><span>{module.key}</span><strong>{module.label}</strong></button>;
    })}
  </nav>;
}
