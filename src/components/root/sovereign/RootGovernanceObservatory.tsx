...
type RootMode = 'observatory' | 'governance';
...
function rowItem(row: RootRow, index: number, source: string, kind: string): ObservableItem {
  const itemId = id(row, `${kind}-${index}`);
  return {
    id: itemId,
...
function itemsForLens(lens: Lens, state: RootSovereignState): ObservableItem[] {
  ...
  if (lens === 'memory') {
    return state.amv.data.memories.slice(0, 140).map((row, index) => rowItem(row, index, state.amv.source, 'memoria'));
  }
  if (lens === 'attractors') {
    return [
  ...
  if (lens === 'agents') {
    return state.agents.data.agents.map((agent) => ({
      ...
      summary: agent.error
        ? `No puede operar completamente: ${agent.error}`
        : text(agent.lastResult, 'Agente registrado. Selecciónalo para revisar su estado y posibles usos.'),
  ...
  const signalRows: ObservableItem[] = [];
  for (const item of state.system.data.matrix) {
    signalRows.push({
      id: item.id,
      title: item.label,
...
export function RootGovernanceObservatory({
  ...
  function generateReport() {
    if (!selected) return;
    const type = lens === 'attractors' || lens === 'memory'
      ? 'amv_recurrence'
      : lens === 'evidence'
  ...
  function runObservationCycle() {
    onAction({
      ...
      effect: 'Ejecuta observación, reportes y auditoría usando WorldSpect y las fuentes persistidas. No requiere validar manualmente cada hipótesis.',
    ...
    <main className="rgo-root">
      <header className="rgo-header">
        <div>
          ...
          <h1>{mode === 'observatory' ? 'Observatorio de gobernanza' : 'Centro de decisiones'}</h1>
          <p>{mode === 'observatory'
            ...
            : 'Único punto para revisar propuestas, permisos, riesgos y cierres que realmente requieren una decisión humana.'}</p>
        </div>
      ...
      {mode === 'governance' ? (
        <section className="rgo-governance" aria-label="Centro de decisiones ROOT">
          <article>
            ...
            <strong>{governanceCounts.proposals}</strong>
            <p>Propuestas registradas que pueden requerir revisión, autorización o cierre.</p>
          </article>
          <article>
            <span>EVIDENCIA PENDIENTE</span>
            <strong>{governanceCounts.pendingEvidence}</strong>
            <p>Solicitudes que todavía necesitan evidencia suficiente antes de convertirse en decisión.</p>
          </article>
          <article>
            <span>RIESGOS Y ADVERTENCIAS</span>
            <strong>{governanceCounts.warnings}</strong>
            <p>Condiciones degradadas, información faltante o límites que impiden una acción segura.</p>
          </article>
          <article>
            <span>CAPACIDADES DISPONIBLES</span>
            <strong>{governanceCounts.capabilities}</strong>
            <p>Operaciones que ROOT puede ejecutar ahora usando sus contratos existentes.</p>
          </article>

          <div className="rgo-decision-stream">
            <header>
              <div><span>DECISIONES ABIERTAS</span><h2>Qué necesita realmente tu atención</h2></div>
              <button type="button" onClick={runObservationCycle}>ACTUALIZAR OBSERVACIÓN Y APRENDIZAJE</button>
            </header>
        ...
        <section className="rgo-observatory" aria-label="Observatorio exploratorio ROOT">
          ...
          <div className="rgo-field-layout">
            <article className="rgo-field">
              <header>
                <div><span>{LENSES.find((item) => item.id === lens)?.label}</span><h2>{LENSES.find((item) => item.id === lens)?.description}</h2></div>
                <div><b>{items.length}</b><small>elementos observables</small></div>
              </header>
              <svg viewBox="0 0 100 62" role="img" aria-label={`Campo exploratorio de ${lens}`}>
                <defs>
                  <radialGradient id="rgoHalo"><stop offset="0" stopColor="currentColor" stopOpacity=".75" /><stop offset="1" stopColor="currentColor" stopOpacity="0" /></radialGradient>
                </defs>
                {items.slice(0, 90).map((item, index) => {
                  ...
                  return <g key={item.id} className={`rgo-point ${selectedId === item.id ? 'selected' : ''}`} role="button" tabIndex={0} onClick={() => choose(item)} onKeyDown={(event) => { if (event.key === 'Enter') choose(item); }}>
                    <circle cx={x} cy={y} r={2.2 + confidence * 2.8} className="rgo-halo" />
                    <circle cx={x} cy={y} r={.45 + confidence * .55} className="rgo-core" />
                    {(selectedId === item.id || items.length < 18) ? <text x={x + 1.5} y={y - 1.2}>{item.title.slice(0, 30)}</text> : null}
                  </g>;
                })}
              </svg>
            ...
            <aside className="rgo-focus">
              ...
              {selected ? <>
                ...
                <dl>
                  <div><dt>Qué es</dt><dd>{selected.kind}</dd></div>
                  <div><dt>Cuándo se observó</dt><dd>{shortDate(selected.observedAt)}</dd></div>
                  <div><dt>Qué tan sólida es la lectura</dt><dd>{selected.confidence === null ? 'Todavía no se ha medido' : `${Math.round(selected.confidence * 100)}%`}</dd></div>
                  <div><dt>Evidencia relacionada</dt><dd>{selected.evidenceIds.length || 'Aún no vinculada'}</dd></div>
                </dl>
                <div className="rgo-actions">
                  <button type="button" onClick={analyzeSelected}>ANALIZAR RELACIONES</button>
                  <button type="button" onClick={generateReport}>GENERAR LECTURA</button>
                </div>
              </> : <div className="rgo-empty"><strong>Selecciona un punto.</strong><p>ROOT mostrará qué significa, qué lo sostiene y qué acciones se pueden desencadenar.</p></div>}
            </aside>
          </div>

          <footer className="rgo-pipeline">
            <span>FLUJO ACTIVO</span>
            <b>SELECCIONAR O CARGAR EVIDENCIA</b><i>→</i><b>OBSERVAR RELACIONES</b><i>→</i><b>EJECUTAR AGENTES</b><i>→</i><b>GENERAR LECTURA</b><i>→</i><b>PERSISTIR O CERRAR</b>
          </footer>
        </section>
      )}
    </main>
