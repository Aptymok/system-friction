...
export function PredictionIntervalPlot({ runs }: { runs: RootRow[] }) {
  ...
  const values = valid.flatMap((item) => [item.lo!, item.hi!, item.point!]); const min = Math.min(...values); const max = Math.max(...values); const scale = (value: number) => max === min ? 50 : ((value - min) / (max - min)) * 100;
  return <div className="rs-intervals">{valid.map(({ run, lo, hi, point }, index) => <div key={String(run.id ?? index)}><span>{String(run.subject ?? run.id ?? index)}</span><i style={{ left: `${scale(lo!)}%`, width: `${Math.max(1, scale(hi!) - scale(lo!))}%` }} /><b style={{ left: `${scale(point!)}%` }} title={`prediction ${point}`} /></div>)}</div>;
}
