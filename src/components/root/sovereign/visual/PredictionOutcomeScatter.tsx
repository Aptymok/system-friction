...
export function PredictionOutcomeScatter({ runs, outcomes }: { runs: RootRow[]; outcomes: RootRow[] }) {
  ...
  const points = outcomes.map((outcome) => {
    ...
    return { id: String(outcome.id), predicted: numeric(run?.prediction), actual: numeric(outcome.observed_value) };
  ...
  const values = points.flatMap((point) => [point.predicted, point.actual]); const min = Math.min(...values); const max = Math.max(...values); const scale = (value: number) => max === min ? 50 : 8 + ((value - min) / (max - min)) * 84;
