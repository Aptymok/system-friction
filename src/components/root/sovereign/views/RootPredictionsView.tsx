...
export function RootPredictionsView({ state, onSelect }: { state: RootSovereignState; onSelect: (selection: RootSelection) => void }) {
  const select = (kind: string, entry: RootRow, source: string) => onSelect({ kind, id: String(entry.id ?? entry.hypothesis_id ?? 'SIN ID'), title: String(entry.subject ?? entry.prediccion_explicita ?? entry.name ?? 'Prediction'), source, observedAt: typeof entry.updated_at === 'string' ? entry.updated_at : typeof entry.created_at === 'string' ? entry.created_at : null, confidence: typeof entry.confidence === 'number' ? entry.confidence : typeof entry.probabilidad_estimativa === 'number' ? entry.probabilidad_estimativa : null, evidenceIds: Array.isArray(entry.evidence_ids) ? entry.evidence_ids as string[] : [], warning: null, data: entry });
  return (
    <section className="rs-view">
      <div className="rs-view-title"><span>PREDICTIONS</span><h1>PREDICTIVE ENGINE</h1><p>Modelos calibrados y contrato manual legacy permanecen separados.</p></div>
      ...
      <div className="rs-prediction-grid">
        ...
        <article><header>CALIBRATION MATRIX</header><CalibrationMatrix models={state.predictions.data.models} /></article>
        ...
        <article><header>RUNS / OUTCOMES</header><div className="rs-card-list">{state.predictions.data.runs.map((run) => <button type="button" key={String(run.id)} onClick={() => select('predictive run', run, 'sfi_predictive_runs')}><span>{String(run.status ?? 'SIN DATO')}</span><strong>{String(run.subject ?? run.id)}</strong><em>{String(run.due_at ?? 'SIN VENTANA').slice(0, 16)}</em></button>)}</div></article>
      </div>
