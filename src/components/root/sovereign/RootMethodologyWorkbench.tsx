...
type MethodologyState = {
  caseId: string;
  title: string;
  disposition: 'LINKED_EXISTING' | 'REGISTERED_AUTOMATICALLY' | 'BLOCKED' | 'NOT_REQUIRED';
...
function dispositionLabel(value: MethodologyState['disposition']) {
  ...
  if (value === 'REGISTERED_AUTOMATICALLY') return 'PPOI AUTOMÁTICO';
...
export function RootMethodologyWorkbench({ state }: { state: RootSovereignState }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/root/methodology', { cache: 'no-store', credentials: 'include' })
      .then(async (response) => {
        ...
        if (active) {
          setPayload(body.data ?? { cases: [], warnings: [] });
          setError(null);
        }
      })
      .catch((cause) => {
        if (active) setError(cause instanceof Error ? cause.message : 'methodology_state_unavailable');
      });
    return () => { active = false; };
  }, [state.generatedAt]);
  ...
  const summary = useMemo(() => ({
    ...
    notRequired: payload.cases.filter((entry) => entry.disposition === 'NOT_REQUIRED').length,
  }), [payload.cases]);
  ...
  return (
    <>
      <button className="rmw-trigger" type="button" onClick={() => setOpen(true)}>
        ...
        <small>{summary.blocked ? `${summary.blocked} bloqueados` : `${summary.ppoi} PPOI automáticos`}</small>
      </button>
      ...
      {open ? (
        <div className="rmw-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
          <section className="rmw-window" role="dialog" aria-modal="true" aria-labelledby="rmw-title">
            <header>
              <div><span>SYSTEM FRICTION INSTITUTE · OPERACIONAL</span><h2 id="rmw-title">Resolución metodológica automática</h2></div>
            ...
            <div className="rmw-summary">
              ...
              <div><strong>{summary.ppoi}</strong><span>PPOI enlazados / registrados</span></div>
                ...
                return (
                  <article key={entry.caseId} data-status={entry.disposition === 'BLOCKED' ? 'blocked' : 'ready'}>
                    <div className="rmw-case-head">
                      <div><span>{entry.caseId}</span><h3>{entry.title}</h3></div>
                      <b>{dispositionLabel(entry.disposition)}</b>
                    </div>
                    <dl>
                      <div><dt>Método primario</dt><dd>{primary}</dd></div>
                      <div><dt>Por qué</dt><dd>{resolution.primary?.reasonCodes.join(', ') || 'Sin resolución suficiente'}</dd></div>
                    ...
                    <p className="rmw-auto-note">La creación o enlace PPOI es responsabilidad del ciclo institucional. ROOT sólo expone el resultado o el bloqueo; no requiere una decisión rutinaria del fundador.</p>
                  </article>
                );
