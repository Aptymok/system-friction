...
function MicroIcon({ kind, tone }: { kind: MetricIcon; tone: Tone }) {
  ...
  const common = { className: cls, viewBox: '0 0 28 28', 'aria-hidden': 'true' } as const;
  if (kind === 'friction') return <svg {...common}><circle cx="14" cy="14" r="9" /><path d="M6 14h16M14 6v16M8 8l12 12M20 8L8 20" /></svg>;
  if (kind === 'prediction') return <svg {...common}><path d="M14 4l9 5v10l-9 5-9-5V9z" /><path d="M5 9l9 5 9-5M14 14v10" /></svg>;
  if (kind === 'evidence') return <svg {...common}><circle cx="7" cy="8" r="2" /><circle cx="21" cy="8" r="2" /><circle cx="14" cy="20" r="2" /><path d="M9 9h10M8 10l5 8M20 10l-5 8" /></svg>;
  if (kind === 'agent') return <svg {...common}><path d="M9 7h10l4 7-4 7H9l-4-7z" /><circle cx="11" cy="14" r="1.5" /><circle cx="17" cy="14" r="1.5" /><path d="M14 5V3M14 25v-2" /></svg>;
  if (kind === 'memory') return <svg {...common}><rect x="6" y="5" width="16" height="18" rx="2" /><path d="M10 9h8M10 13h8M10 17h5" /></svg>;
  return <svg {...common}><circle cx="14" cy="14" r="3" /><path d="M14 3v5M14 20v5M3 14h5M20 14h5M6.2 6.2l3.5 3.5M18.3 18.3l3.5 3.5M21.8 6.2l-3.5 3.5M9.7 18.3l-3.5 3.5" /></svg>;
}
...
function SignalLine({ tone = 'watch', variant = 0 }: { tone?: Tone; variant?: number }) {
  ...
  return <svg viewBox="0 0 150 28" className={`signal-line ${toneClass(tone)}`} aria-hidden="true"><path d={paths[variant % paths.length]} fill="none" stroke="currentColor" strokeWidth="1.05" /><path d="M2 25 L148 25" stroke="currentColor" strokeOpacity="0.16" /></svg>;
}
...
function MetricInstrument({ label, value, meta, tone, icon, index }: { label: string; value: string; meta: string; tone: Tone; icon: MetricIcon; index: number }) {
  return <div className="instrument"><MicroIcon kind={icon} tone={tone} /><div className="instrument-main"><div className="instrument-label">{label}</div><div className="instrument-value-row"><b className={toneClass(tone)}>{value}</b><span>{meta}</span></div><div className="micro-meta"><span>vector {index + 1}</span><span>{tone}</span><span>trace live</span></div></div><SignalLine tone={tone} variant={index} /></div>;
}
...
function FloatingList({ title, items, mode }: { title: string; items: unknown[]; mode?: 'actions' | 'log' }) {
  return <div className="float-panel list-panel"><header><b>{title}</b><span>{items.length} records</span></header><div className="list-stack">{items.slice(0, 6).map((item, index) => { const record = asRecord(item); const label = text(record.action ?? record.hypothesis_id ?? record.id ?? record.type ?? record.status, `SFI-${index + 1}`); const body = text(record.reason ?? record.prediccion_explicita ?? record.prediction ?? record.summary ?? record.expected_outcome ?? record.status, 'sin descripcion'); const tone = statusTone(record.status ?? record.evidence_state ?? record.estado_observacion ?? record.current_signal_state); return <div className="list-row" key={`${title}-${index}`}><div><span className={`dot ${toneClass(tone)}`} /><b>{label}</b><p>{body}</p></div>{mode === 'actions' ? <div className="row-actions"><button>INSPECT</button><button>ROUTE</button></div> : <span className={`tag ${toneClass(tone)}`}>{tone}</span>}</div>; })}{items.length === 0 ? <div className="empty">No hay registros disponibles para este hub.</div> : null}</div></div>;
}

function MicroPanel({ title, value, tone, children }: { title: string; value: string; tone: Tone; children?: ReactNode }) {
  return <div className="micro-panel"><div><span>{title}</span><b className={toneClass(tone)}>{value}</b></div>{children}</div>;
}

function CoreTopology({ confidence, graphNodes, evidenceCount, predictionCount, amvCount, queueCount, warningCount, agentCount }: { confidence: number; graphNodes: number; evidenceCount: number; predictionCount: number; amvCount: number; queueCount: number; warningCount: number; agentCount: number }) {
  const model = useMemo(() => buildRootTopologyModel({ confidence, graphNodeCount: graphNodes, evidenceCount, predictionCount, amvCount, queueCount, warningCount, agentCount }), [agentCount, amvCount, confidence, evidenceCount, graphNodes, predictionCount, queueCount, warningCount]);
  const byId = useMemo(() => Object.fromEntries(model.nodes.map((node) => [node.id, node])) as Record<RootTopologyNodeId, (typeof model.nodes)[number]>, [model.nodes]);

  return <div className="core-stage"><div className="stage-title"><span>INTERNAL OBSERVATORY</span><b>ROOT OBSERVING SELF</b></div><div className="stage-corners"><i /><i /><i /><i /></div><div className="stage-side left"><span>FIELD</span><span>MEMORY</span><span>HYPOTHESIS</span></div><div className="stage-side right"><span>AGENTS</span><span>QUEUE</span><span>WORLD</span></div><svg viewBox="0 0 100 100" className="core-svg" aria-hidden="true"><defs><radialGradient id="rootGlowContract" cx="50%" cy="50%" r="52%"><stop offset="0%" stopColor="#f4d987" stopOpacity="0.42" /><stop offset="46%" stopColor="#c8a951" stopOpacity="0.10" /><stop offset="100%" stopColor="#000000" stopOpacity="0" /></radialGradient><filter id="softGlowContract"><feGaussianBlur stdDeviation="0.55" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter></defs><circle cx="50" cy="50" r="47" fill="url(#rootGlowContract)" />{model.rings.map((ring, i) => <circle key={ring.id} cx="50" cy="50" r={ring.radius} fill="none" stroke="#c8a951" strokeOpacity={0.12 + ring.weight * 0.30} strokeWidth={i === 3 ? '0.32' : '0.2'} />)}{Array.from({ length: 36 }, (_, i) => { const a = (i * 10) * Math.PI / 180; const x1 = 50 + Math.cos(a) * (i % 2 ? 40 : 44); const y1 = 50 + Math.sin(a) * (i % 2 ? 40 : 44); const x2 = 50 + Math.cos(a) * 48; const y2 = 50 + Math.sin(a) * 48; return <line key={`tick-${i}`} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#c8a951" strokeOpacity={i % 3 ? '0.15' : '0.28'} strokeWidth="0.16" />; })}{Array.from({ length: 24 }, (_, i) => { const a = (i * 15) * Math.PI / 180; const x1 = 50 + Math.cos(a) * 7; const y1 = 50 + Math.sin(a) * 7; const x2 = 50 + Math.cos(a) * 47; const y2 = 50 + Math.sin(a) * 47; return <line key={`ray-${i}`} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#c8a951" strokeOpacity="0.10" strokeWidth="0.16" />; })}{model.microSignals.map((signal) => <circle key={signal.id} cx={signal.x} cy={signal.y} r={signal.radius} fill="#f0cf78" opacity={signal.opacity} />)}{model.edges.map((edge) => { const from = byId[edge.from]; const to = byId[edge.to]; const opacity = edge.kind === 'cycle' ? edge.weight : edge.kind === 'cross' ? edge.weight * 0.75 : edge.weight * 0.58; const width = edge.kind === 'cycle' ? '0.34' : edge.kind === 'cross' ? '0.20' : '0.16'; return <line key={edge.id} x1={from.x} y1={from.y} x2={to.x} y2={to.y} stroke={edge.kind === 'cross' ? '#f0cf78' : '#c8a951'} strokeOpacity={opacity} strokeWidth={width} />; })}{model.nodes.map((node) => <g key={node.id} filter="url(#softGlowContract)"><circle cx={node.x} cy={node.y} r="4.2" fill="#060504" stroke="#f0cf78" strokeOpacity="0.88" strokeWidth="0.40" /><circle cx={node.x} cy={node.y} r="2.15" fill="none" stroke="#c8a951" strokeOpacity="0.26" strokeWidth="0.18" /><text x={node.x} y={node.y - 0.8} textAnchor="middle" fill="#f0cf78" fontSize="1.7" fontFamily="monospace">{node.label}</text><text x={node.x} y={node.y + 2.15} textAnchor="middle" fill="#e7dcc1" fontSize="1.85" fontFamily="monospace">{node.value}</text></g>)}<circle cx="50" cy="50" r="10" fill="#050403" stroke="#f0cf78" strokeWidth="0.50" /><circle cx="50" cy="50" r="6.8" fill="none" stroke="#c8a951" strokeOpacity="0.42" strokeWidth="0.24" /><circle cx="50" cy="50" r="3.1" fill="none" stroke="#f0cf78" strokeOpacity="0.22" strokeWidth="0.18" /><text x="50" y="48.7" textAnchor="middle" fill="#f0cf78" fontSize="3.0" fontFamily="monospace">SFI</text><text x="50" y="52.6" textAnchor="middle" fill="#a99d82" fontSize="1.85" fontFamily="monospace">CORE</text></svg><div className="stage-readout"><span>CONF {pct(confidence)}</span><span>GRAPH {graphNodes}</span><span>EVID {evidenceCount}</span><span>PRED {predictionCount}</span><span>AMV {amvCount}</span></div><div className="stage-contract"><b>TOPOLOGY CONTRACT</b><span>nodes {model.nodes.length}</span><span>edges {model.edges.length}</span><span>rings {model.rings.length}</span><span>micro {model.microSignals.length}</span></div></div>;
}
...
export default function SfiRootLiveConsole({ initialState, worldVector }: { initialState: AgenticRootState; worldVector: unknown }) {
  ...
  const hubs = [{ id: 'total' as const, label: 'Estado total', glyph: '◎' }, { id: 'predictions' as const, label: 'Predicciones', glyph: '◇' }, { id: 'agents' as const, label: 'Agentes', glyph: '✦' }, { id: 'founder' as const, label: 'Fundador', glyph: '⬡' }, { id: 'world' as const, label: 'World Vector', glyph: '⌁' }];
