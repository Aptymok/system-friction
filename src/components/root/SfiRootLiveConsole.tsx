...
function CoreTopology({ confidence, graphNodes, evidenceCount, predictionCount, amvCount }: { confidence: number; graphNodes: number; evidenceCount: number; predictionCount: number; amvCount: number }) {
  ...
  const microNodes = useMemo(() => Array.from({ length: 42 }, (_, i) => {
    const angle = (i * 137.5) * Math.PI / 180;
    const radius = 12 + ((i * 7) % 31);
    return { x: 50 + Math.cos(angle) * radius, y: 50 + Math.sin(angle) * radius, r: 0.34 + (i % 4) * 0.08, opacity: 0.16 + (i % 5) * 0.055 };
  }), []);
  ...
  return (
    <div className="core-stage">
      ...
      <svg viewBox="0 0 100 100" className="core-svg" aria-hidden="true">
        <defs>
          <radialGradient id="rootGlowDense" cx="50%" cy="50%" r="52%">
            <stop offset="0%" stopColor="#f4d987" stopOpacity="0.42" />
            <stop offset="46%" stopColor="#c8a951" stopOpacity="0.10" />
            <stop offset="100%" stopColor="#000000" stopOpacity="0" />
          </radialGradient>
          <filter id="softGlow"><feGaussianBlur stdDeviation="0.55" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
        </defs>
        <circle cx="50" cy="50" r="47" fill="url(#rootGlowDense)" />
        {[8, 14, 20, 26, 32, 38, 44, 48].map((r, i) => <circle key={r} cx="50" cy="50" r={r} fill="none" stroke="#c8a951" strokeOpacity={i % 2 ? '0.18' : '0.30'} strokeWidth={i === 4 ? '0.34' : '0.22'} />)}
        {Array.from({ length: 18 }, (_, i) => {
          ...
          return <line key={`ray-${i}`} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#c8a951" strokeOpacity="0.12" strokeWidth="0.18" />;
        })}
        ...
        {nodes.map((node, index) => {
          ...
          return <g key={`edge-${node.label}`}><line x1={node.x} y1={node.y} x2={next.x} y2={next.y} stroke="#c8a951" strokeOpacity="0.50" strokeWidth="0.36" /><line x1={node.x} y1={node.y} x2={cross.x} y2={cross.y} stroke="#f0cf78" strokeOpacity="0.18" strokeWidth="0.22" /><line x1={node.x} y1={node.y} x2={far.x} y2={far.y} stroke="#c8a951" strokeOpacity="0.11" strokeWidth="0.18" /><line x1="50" y1="50" x2={node.x} y2={node.y} stroke="#c8a951" strokeOpacity="0.30" strokeWidth="0.26" /></g>;
        })}
        {nodes.map((node) => <g key={node.label} filter="url(#softGlow)"><circle cx={node.x} cy={node.y} r="4.4" fill="#060504" stroke="#f0cf78" strokeOpacity="0.88" strokeWidth="0.42" /><circle cx={node.x} cy={node.y} r="2.2" fill="none" stroke="#c8a951" strokeOpacity="0.28" strokeWidth="0.2" /><text x={node.x} y={node.y - 0.8} textAnchor="middle" fill="#f0cf78" fontSize="1.8" fontFamily="monospace">{node.label}</text><text x={node.x} y={node.y + 2.25} textAnchor="middle" fill="#e7dcc1" fontSize="1.9" fontFamily="monospace">{node.value}</text></g>)}
        <circle cx="50" cy="50" r="10" fill="#050403" stroke="#f0cf78" strokeWidth="0.52" />
        <circle cx="50" cy="50" r="6.8" fill="none" stroke="#c8a951" strokeOpacity="0.42" strokeWidth="0.25" />
        <circle cx="50" cy="50" r="3.1" fill="none" stroke="#f0cf78" strokeOpacity="0.22" strokeWidth="0.2" />
