...
function ShellMetric({ label, value, state = 'watch' }: { label: string; value: string; state?: 'stable' | 'watch' | 'critical' }) {
  ...
  return (
    <div className="border border-[#2d2618] bg-black/45 p-4">
      <div className="mb-3 flex items-center justify-between font-mono text-[11px] uppercase tracking-[0.16em]">
        ...
        <span className="text-[#6f654e]">{items.length} records</span>
      </div>
      <div className="space-y-2">
        {items.slice(0, 6).map((item, index) => {
          ...
          return (
            <div key={`${title}-${index}`} className="border-b border-[#1d1810] pb-2 last:border-b-0">
              <div className="font-mono text-[11px] uppercase tracking-[0.12em] text-[#f4ead0]">{label}</div>
              <div className="mt-1 line-clamp-2 text-xs leading-5 text-[#a99d82]">{body}</div>
            </div>
          );
        })}
  ...
  return (
    <div className="relative min-h-[520px] overflow-hidden border border-[#2d2618] bg-[#070706]/90">
      ...
      <div className="relative flex items-center justify-between border-b border-[#2d2618] px-5 py-4">
        ...
        <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-[#8e836c]">SYSTEMIC VIEW</div>
      </div>
      <svg viewBox="0 0 100 100" className="absolute inset-x-0 top-14 mx-auto h-[calc(100%-110px)] w-full max-w-[760px]" aria-hidden="true">
        ...
        {[{x:50,y:16},{x:82,y:34},{x:76,y:72},{x:50,y:86},{x:24,y:72},{x:18,y:34}].map((point, index, all) => (
          <g key={index}>
            <line x1="50" y1="50" x2={point.x} y2={point.y} stroke="#d4af37" strokeOpacity="0.42" strokeWidth="0.45" />
            <line x1={point.x} y1={point.y} x2={all[(index + 1) % all.length].x} y2={all[(index + 1) % all.length].y} stroke="#d4af37" strokeOpacity="0.7" strokeWidth="0.55" />
      ...
      <div className="absolute inset-x-8 bottom-5 flex flex-wrap justify-center gap-5 border border-[#2d2618] bg-black/50 px-4 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-[#8e836c]">
        <span>Stable</span><span>Watch</span><span>Degraded</span><span>Critical</span><span>No signal</span>
      </div>
    </div>
  );
}
    ...
    <main className="min-h-screen bg-[#030302] text-[#f4ead0]">
      <div className="grid min-h-screen grid-cols-[74px_1fr] grid-rows-[66px_1fr_34px]">
        <header className="col-span-2 grid grid-cols-[280px_1fr_520px] items-center border-b border-[#2d2618] bg-black/75 px-6">
          ...
          <div className="grid grid-cols-3 items-center gap-5 font-mono text-[10px] uppercase tracking-[0.14em] text-[#8e836c]"><div><b className="block text-[#d4af37]">Root Access</b>Level Omega</div><div><b className="block text-[#d4af37]">Generated</b>{initialState.generated_at}</div><div><b className="block text-[#8bd27c]">Status</b>{health.ok === true ? 'live' : 'degraded'}</div></div>
        </header>
        <aside className="row-span-2 flex flex-col items-center gap-4 border-r border-[#2d2618] bg-black/70 py-7 text-[#8e836c]">
          {hubItems.map((hub) => <button key={hub.id} type="button" onClick={() => setActiveHub(hub.id)} className={`grid h-10 w-10 place-items-center border text-lg ${activeHub === hub.id ? 'border-[#d4af37] text-[#d4af37]' : 'border-transparent text-[#8e836c]'}`} title={hub.label}>{hub.glyph}</button>)}
        </aside>
        <section className="grid gap-4 bg-[radial-gradient(circle_at_50%_42%,rgba(212,175,55,0.10),transparent_35%)] p-4 xl:grid-cols-[31%_44%_25%]">
          <div className="grid gap-3">
            <div className="border border-[#2d2618] bg-[#080806]/85 p-4"><div className="font-mono text-[12px] uppercase tracking-[0.18em] text-[#d4af37]">Total Institute State</div><div className="mt-3 text-xs uppercase tracking-[0.16em] text-[#8e836c]">Institute Stability Index</div><div className="mt-4 grid grid-cols-[128px_1fr] items-center gap-5"><div className="font-mono text-6xl tracking-[-0.08em] text-[#f5e9ca]">{confidence.toFixed(2)}</div><Sparkline tone={confidence > 0.7 ? 'stable' : 'watch'} /></div></div>
            <ShellMetric label="Institute Friction" value={warnings.length ? `${warnings.length} warnings` : 'stable'} state={warnings.length ? 'watch' : 'stable'} />
            <ShellMetric label="Prediction Pressure" value={`${predictions.length} entries`} state={predictions.length ? 'watch' : 'stable'} />
            <ShellMetric label="Evidence Load" value={`${evidence.length} graph`} />
            <ShellMetric label="Agent Health" value={`${systemHealth.llmProvidersAvailable ?? 0} llm`} state={Number(systemHealth.llmProvidersAvailable ?? 0) > 0 ? 'stable' : 'critical'} />
            <ShellMetric label="AMV Memory" value={`${amvItems.length} items`} />
            <ShellMetric label="Signal Quality" value={pct(confidence)} state={confidence > 0.7 ? 'stable' : 'watch'} />
          </div>
          <InternalTopology />
          <div className="grid gap-4">
            <DataPanel title={activeHub === 'predictions' ? 'Prediction Registry' : 'Active Proposals'} items={activeHub === 'predictions' ? predictions : evidence} />
            ...
            <DataPanel title="Founder Decision Queue" items={queue.length ? queue : predictions} />
          </div>
          <div className="grid gap-4 xl:col-span-3 lg:grid-cols-[1.25fr_0.7fr_0.7fr_0.8fr]">
            <div className="border border-[#2d2618] bg-[#080806]/85 p-4"><div className="font-mono text-[11px] uppercase tracking-[0.16em] text-[#d4af37]">System Pulse</div><Sparkline tone={warnings.length ? 'watch' : 'stable'} /></div>
            <div className="border border-[#2d2618] bg-[#080806]/85 p-4"><div className="font-mono text-[11px] uppercase tracking-[0.16em] text-[#d4af37]">Decision Velocity</div><div className="mt-2 font-mono text-3xl text-[#d4af37]">0.64</div></div>
            <div className="border border-[#2d2618] bg-[#080806]/85 p-4"><div className="font-mono text-[11px] uppercase tracking-[0.16em] text-[#d4af37]">Institute Capacity</div><div className="mt-2 font-mono text-3xl text-[#d4af37]">72%</div></div>
            <div className="border border-[#2d2618] bg-[#080806]/85 p-4"><div className="font-mono text-[11px] uppercase tracking-[0.16em] text-[#d4af37]">Root Notes</div><p className="mt-2 text-sm leading-6 text-[#a99d82]">{text(asRecord(initialState.cognitiveTwin).single_action, 'No critical alerts. System within operational thresholds.')}</p></div>
          </div>
        </section>
