...
function number(value: unknown) { const parsed=typeof value==='number'?value:Number(value); return Number.isFinite(parsed)?(parsed>1?parsed/100:parsed):null; }
...
export function PredictionCasesConsole({ runs, legacy, outcomes, attractors }: { runs:RootRow[];legacy:RootRow[];outcomes:RootRow[];attractors:RootRow[] }) {
  ...
  const attractorByPrediction=useMemo(()=>new Map(attractors.map((a)=>[text(a.owner_node_key).replace(/^prediction:/,''),a])),[attractors]);
  const items=useMemo<Item[]>(()=>[
    ...runs.map((row)=>{const id=text(row.id);const vector=rec(attractorByPrediction.get(id)?.vector);return{id,title:text(row.prediction,text(row.target_key,'Hipótesis')),status:text(row.status,'MISSING'),confidence:number(row.confidence),createdAt:text(row.created_at)||null,origin:text(vector.origin,'REQUESTED_OR_UNKNOWN'),source:'predictive' as const,outcomeCount:outcomes.filter((o)=>text(o.run_id)===id).length,evidenceCount:strings(row.evidence_refs).length,raw:row}}),
    ...legacy.map((row)=>{const id=text(row.id);const vector=rec(attractorByPrediction.get(id)?.vector);return{id,title:text(row.prediccion_explicita,text(row.case_label,'Hipótesis legacy')),status:text(row.estado_observacion??row.evidence_state,'MISSING'),confidence:number(row.probabilidad_estimativa),createdAt:text(row.prediction_registered_at??row.created_at)||null,origin:text(vector.origin,'LEGACY_REGISTRY'),source:'legacy' as const,outcomeCount:0,evidenceCount:0,raw:row}}),
  ].sort((a,b)=>new Date(b.createdAt??0).valueOf()-new Date(a.createdAt??0).valueOf()),[runs,legacy,outcomes,attractorByPrediction]);
  const visible=items.filter((item)=>filter==='all'||filter==='legacy'?filter==='all'||item.source==='legacy':filter==='autonomous'?item.origin.startsWith('SFI_AUTONOMOUS'):!item.origin.startsWith('SFI_AUTONOMOUS')&&item.source!=='legacy');

  return <main className="pc-root"><header><div><span>SFI · ROOT · PREDICTIVE MEMORY</span><h1>PREDICTION CASES</h1><p>Cada caso conserva hipótesis, confianza original, origen, atractor, World Vector del momento, evidencia solicitada, outcome y aprendizaje.</p></div><Link href="/root">VOLVER A ROOT</Link></header>
