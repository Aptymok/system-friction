...
export function ProjectionRegistryView({ trace }: { trace: StudioPipelineTrace | null }) {
  const stage = trace?.stages.find((item) => item.id === 'projection_registry');
  return (
    <section className="rounded-[2rem] border border-[#2d2a28] bg-[#080706]/90 p-5">
      <div className="text-xs uppercase tracking-[0.22em] text-[#9c8c70]">ProjectionRegistryView</div>
