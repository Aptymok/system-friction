...
export default function SfiLabClient() {
  ...
  async function analyze() {
    ...
    try {
      const response = await fetch('/api/sfi/lab/analyze', {
        method: 'POST',
  ...
  async function generateReport() {
    ...
    try {
      const response = await fetch('/api/sfi/lab/report', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
  ...
  async function generateMediaPlan() {
    ...
    try {
      const response = await fetch('/api/sfi/lab/media-plan', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ analysis }),
      });
  ...
  return (
    <main className="min-h-screen bg-black text-[#f4efe4]">
      <form
        action="/api/sfi/lab/analyze"
        method="post"
        ...
        className="mx-auto grid max-w-7xl gap-5 px-4 py-6 lg:grid-cols-[390px_minmax(0,1fr)]"
      >
        <aside className="space-y-4">
          <div className="border border-[#3a3020] bg-[#090908] p-5">
            ...
            <h1 className="mt-3 text-3xl font-semibold text-white">SFI Lab</h1>
