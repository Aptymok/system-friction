...
function proposalKey(proposal: Record<string, unknown>) {
  const delta = proposal.expected_field_delta && typeof proposal.expected_field_delta === 'object' && !Array.isArray(proposal.expected_field_delta)
    ...
    : {};
  return typeof delta.recommendationKey === 'string' ? delta.recommendationKey : 'UNKEYED_PROPOSAL';
}
...
export default async function RootDiscoveryPage() {
  ...
  return <main className="discoveryRoot">
    ...
    <section className="discoveryMetricsGrid" aria-label="Discovery summary">
      ...
      <Metric label="Propuestas de desarrollo" value={data.autonomy.openDevelopmentProposals} detail={data.autonomy.availability} />
    ...
    <section className="discoveryPanel discoveryWide">
      <div className="discoverySectionHead"><div><p className="discoveryKicker">AUTO-OBSERVACIÓN / DEVELOPMENT</p><h2>El Mesh puede observarse y proponer; no puede aprobarse.</h2></div><span className={statusClass(data.autonomy.availability)}>{data.autonomy.availability}</span></div>
      <p>{latestDiscoveryRun ? 'Existe una ejecución Discovery persistida en la muestra actual.' : 'Todavía no existe una ejecución Discovery persistida en la muestra actual.'} Las degradaciones y métricas observadas pueden convertirse en propuestas reversibles dentro de <code>action_proposals</code>; no adquieren autoridad de ejecución ni cambian canon.</p>
      <div className="discoveryGrid">
        <article className="discoveryPanel"><h2>Desarrollo propuesto</h2>{data.autonomy.developmentProposals.length ? <div className="discoveryTable" role="table">{data.autonomy.developmentProposals.slice(0, 8).map((proposal) => <div className="discoveryRow" role="row" key={String(proposal.id)}><div><strong>{String(proposal.title ?? 'Propuesta')}</strong><small>{proposalKey(proposal)}</small></div><span className={statusClass(String(proposal.status ?? 'UNKNOWN'))}>{String(proposal.status ?? 'UNKNOWN')}</span><div className="discoveryReason">{String(proposal.objective ?? proposal.description ?? '')}</div></div>)}</div> : <p className="discoveryBoundary">Sin propuestas todavía. La ausencia no se interpreta como salud ni como cero.</p>}</article>
        <article className="discoveryPanel"><h2>Editorial derivado</h2>{data.autonomy.editorialProposals.length ? <div className="discoveryTable" role="table">{data.autonomy.editorialProposals.slice(0, 4).map((proposal) => <div className="discoveryRow" role="row" key={String(proposal.id)}><div><strong>{String(proposal.title ?? 'Candidato editorial')}</strong><small>{proposalKey(proposal)}</small></div><span className={statusClass(String(proposal.status ?? 'UNKNOWN'))}>{String(proposal.status ?? 'UNKNOWN')}</span><div className="discoveryReason">{String(proposal.objective ?? proposal.description ?? '')}</div></div>)}</div> : <p className="discoveryBoundary">El siguiente ciclo puede proponer una nota a partir de una observación persistida. Candidato ≠ publicación.</p>}<p><Link href="/publications/discovery-mesh-publicar-no-es-ser-encontrado">ABRIR NOTA DE LABORATORIO AUTORIZADA →</Link></p></article>
      </div>
