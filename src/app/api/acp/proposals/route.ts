...
export async function GET() {
  const gate=await requireGovernedActor('acp.proposals.list'); if(!gate.ok)return NextResponse.json(gate.body,{status:gate.status}); if(!gate.ctx.isRoot)return NextResponse.json({ok:false,error:'root_required'},{status:403});
  const proposals=await latestActionProposals(undefined,50); if(proposals.error)return NextResponse.json({ok:false,error:'proposal_list_failed',details:proposals.error},{status:500});
  const rows=proposals.data.map(row=>summarizeProposal(asRecord(row)));
  return NextResponse.json({ok:true,data:{proposals:rows,counts:{total:rows.length,proposed:rows.filter(row=>row.status==='proposed').length,waitingEvidence:rows.filter(row=>row.status==='waiting_evidence').length,designApproved:rows.filter(row=>row.status==='design_approved').length,queued:rows.filter(row=>row.status==='queued').length,accepted:rows.filter(row=>row.status==='accepted').length,conflicted:rows.filter(row=>row.status==='conflicted').length,frozen:rows.filter(row=>row.status==='frozen').length,rejected:rows.filter(row=>row.status==='rejected').length}}});
}
