...
async function activateCheckoutSession(session: Stripe.Checkout.Session) {
  ...
  const { error } = await service.from('sfi_user_entitlements').upsert({
    ...
    source: session.payment_link ? 'stripe_payment_link' : 'stripe_checkout',
    metadata: {
      checkout_session_id: session.id,
      payment_status: session.payment_status,
      payment_link_id: objectId(session.payment_link),
      mode: session.mode,
    },
...
export async function POST(request: Request) {
  ...
  const webhookSecret = process.env.STRIPE_WEBHOOK_SECRET || process.env.STRIPE_INTERFACE_WEBHOOK_SECRET;
  ...
  if (event.type === 'checkout.session.completed' || event.type === 'checkout.session.async_payment_succeeded') {
    const error = await activateCheckoutSession(event.data.object as Stripe.Checkout.Session);
  ...
  if (event.type === 'invoice.payment_failed') {
    ...
    if (subscriptionId) {
      const service = createServiceSupabaseClient();
      const { error } = await service.from('sfi_user_entitlements').update({
        status: 'past_due',
