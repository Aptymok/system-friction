...
export async function POST(req: Request, ctx: RouteContext) {
  ...
  if (!rationale) return NextResponse.json({ ok: false, error: 'rationale_required' }, { status: 400 });
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .update({
        ...
        outcome: {
          closure: 'obsolete_against_active_attractor',
          rationale,
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : 'close_obsolete_failed' }, { status: 400 });
  }
}
