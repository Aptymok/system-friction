...
export async function POST(request:Request){
  ...
  const operatingCycleId=text(body.operatingCycleId); const primaryHypothesis=text(body.primaryHypothesis);
  ...
  const rivals=list(body.rivalHypotheses).filter(value=>value!==primaryHypothesis); const discriminators=list(body.discriminatingObservations); const unknowns=list(body.unknowns);
  const status=discriminators.length&&rivals.length?'CONTRAST_READY':'OPEN';
  const inserted=await gate.ctx.service.from('sfi_inference_traces').insert({ operating_cycle_id:operatingCycleId, owner_id:gate.ctx.user.id, question:text(cycle.data.question), primary_hypothesis:primaryHypothesis, rival_hypotheses:rivals, evidence_refs:evidenceRefs, unknowns, discriminating_observations:discriminators, stopping_condition:text(body.stoppingCondition)||null, epistemic_class:'INFERRED', status, payload:{rule:'Inference is not observation. A hypothesis remains INFERRED until a discriminating observation is actually acquired and linked.'} }).select('*').single();
