...
const REF_COLUMNS: Record<string,string> = { evidence:'evidence_refs', studio:'studio_object_refs', method_lab:'method_lab_refs', inference:'inference_refs', trajectory:'trajectory_refs', return:'return_refs', twin:'cognitive_twin_refs', governance:'governance_refs', event:'event_refs' };
...
function cycleCode() { const stamp = new Date().toISOString().replaceAll('-', '').replaceAll(':', '').replaceAll('T', '').replaceAll('Z', '').replaceAll('.', '').slice(0,14); return `SFI-CYCLE-${stamp}-${crypto.randomUUID().slice(0,8).toUpperCase()}`; }

export async function GET() {
  const gate = await requireRootActor('root.operate.read');
  if (!gate.ok) return NextResponse.json(gate.body, { status: gate.status });
  const result = await gate.ctx.service.from('sfi_operating_cycles').select('*').eq('owner_id', gate.ctx.user.id).order('updated_at',{ascending:false}).limit(50);
...
export async function POST(request: Request) {
  ...
  const title = text(body.title); const question = text(body.question);
  const subjectRaw = text(body.subject).toUpperCase() as MihmObservationSubject; const temporalRaw = text(body.temporalScope).toUpperCase() as MihmTemporalScope;
  const subject: MihmObservationSubject = SUBJECTS.has(subjectRaw) ? subjectRaw : 'UNKNOWN'; const temporalScope: MihmTemporalScope = TEMPORAL.has(temporalRaw) ? temporalRaw : 'UNKNOWN';
  ...
  const created = await gate.ctx.service.from('sfi_operating_cycles').insert({ cycle_code:cycleCode(), owner_id:gate.ctx.user.id, title, question, subject, temporal_scope:temporalScope, status:'OPEN', metadata:{createdFrom:'root_operating_field', evidenceModalities} }).select('*').single();
  ...
  const method = resolveMihmMethod({ subject, temporalScope, evidenceModalities, subjectId:['OBJECT','SIGNAL','ARTIFACT','PHENOMENON','CASE','ORGANIZATION'].includes(subject) ? cycleId : null, sessionId:['PERSON','SESSION'].includes(subject) ? cycleId : null, caseId:['CASE','ORGANIZATION'].includes(subject) ? cycleId : null, phenomenonId:subject==='PHENOMENON' ? cycleId : null, ownerId:gate.ctx.user.id, evidenceCount:evidenceModalities.length, worldContextRequested:Boolean(body.worldContextRequested), requiresTrajectory:Boolean(body.requiresTrajectory) || temporalScope==='LONGITUDINAL', requiresRivalHypothesis:Boolean(body.requiresRivalHypothesis), requiresInterventionTracking:Boolean(body.requiresInterventionTracking), isSfiInternal:subject==='SFI_SYSTEM' });
  const nextStatus = method.status==='READY' ? 'METHOD_SELECTED' : 'OPEN';
  const updated = await gate.ctx.service.from('sfi_operating_cycles').update({ method_resolution:method, status:nextStatus, updated_at:new Date().toISOString() }).eq('id',cycleId).eq('owner_id',gate.ctx.user.id).select('*').single();
...
export async function PATCH(request: Request) {
  ...
  if (!gate.ok) return NextResponse.json(gate.body,{status:gate.status});
  const body = await request.json().catch(()=>({})) as Row; const id = text(body.id);
  ...
  const patch: Row = {updated_at:new Date().toISOString()}; const status = text(body.status).toUpperCase(); if (status) patch.status=status;
  const refKind = text(body.refKind).toLowerCase(); const ref = text(body.ref);
  if (refKind && ref && REF_COLUMNS[refKind]) { const column=REF_COLUMNS[refKind]; patch[column]=Array.from(new Set([...list(current.data[column]),ref])); }
  if (text(body.fieldCaseRef)) patch.field_case_ref=text(body.fieldCaseRef); if (status==='CLOSED') patch.closed_at=new Date().toISOString();
