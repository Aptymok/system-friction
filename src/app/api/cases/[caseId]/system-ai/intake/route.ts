...
const executionSchema=z.object({type:z.literal('AI_EXECUTION'),executionId:z.string().trim().min(1).max(240),aiSystemRef:aiSystemRefSchema,modelRef:aiModelRefSchema,startedAt:z.string().trim().min(1).max(80),finishedAt:z.string().trim().min(1).max(80),status:z.string().trim().min(1).max(120),promptTemplateRef:promptRefSchema.nullable().optional(),dataSourceRefs:z.array(dataRefSchema).max(100).optional(),toolRefs:z.array(toolRefSchema).max(100).optional(),inputHash:z.string().trim().max(256).nullable().optional(),contextHash:z.string().trim().max(256).nullable().optional(),outputHash:z.string().trim().max(256).nullable().optional(),sourceRefs:z.array(refSchema).max(500).optional()}).strict();
...
export async function POST(request:Request,context:RouteContext){
  try{
    ...
    return NextResponse.json({
      ...
      ...result,
      boundary:'System/AI assurance intake accepts completed execution traces and validates the full packet before persisting object + relations in one database transaction. Raw prompts/inputs are not required; AI output does not become a decision, failure does not establish root cause, and no institutional truth is written.',
    },{status:201});
  }catch(error){
