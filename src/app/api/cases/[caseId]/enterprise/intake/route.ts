...
import { assertCaseReferenceIntegrity, assertCaseServiceProfileAllowed, assertTenantEnterpriseEntityRefs } from '@/lib/sfi/case-platform/integrity';
...
async function persistPackage(caseId:string,userId:string,packet:SfiEnterpriseIntakePackage){
  ...
  const object=await recordOperationalCaseObject({caseId,userId,...packet.object}); const relations=[];
  for(const relation of packet.relations){await assertCaseReferenceIntegrity({caseId,userId,sourceRefs:relation.sourceRefs,recordRefs:relation.recordRefs,evidenceRefs:relation.evidenceRefs});await assertTenantEnterpriseEntityRefs({caseId,userId,entityRefs:[relation.from,relation.to]});relations.push(await recordOperationalEnterpriseRelation({caseId,userId,relation}));}
  return{object,relations};
}

export async function POST(request:Request,context:RouteContext){
  try{
    const{user}=await requireAuthenticatedUser(); const{caseId}=await context.params; const body=intakeSchema.parse(await request.json()); let packet:SfiEnterpriseIntakePackage;
    if(body.type==='SERVICE_TICKET'){
      await assertCaseServiceProfileAllowed(caseId,user.id,['SERVICE_OBSERVABILITY','ENTERPRISE_MEMORY','CUSTOM_RESEARCH']); packet=normalizeServiceTicketRecord(body);
    }else if(body.type==='WARRANTY_EVENT'){
      await assertCaseServiceProfileAllowed(caseId,user.id,['CONTRACT_WARRANTY_ASSURANCE','ENTERPRISE_MEMORY','CUSTOM_RESEARCH']); packet=normalizeWarrantyEventRecord(body);
    }else if(body.type==='TENDER_REQUIREMENT'){
      await assertCaseServiceProfileAllowed(caseId,user.id,['TENDER_ASSURANCE','ENTERPRISE_MEMORY','CUSTOM_RESEARCH']); packet=normalizeTenderRequirementRecord(body);
    }else{
      await assertCaseServiceProfileAllowed(caseId,user.id,['SERVICE_OBSERVABILITY','CONTRACT_WARRANTY_ASSURANCE','TENDER_ASSURANCE','ENTERPRISE_MEMORY','CUSTOM_RESEARCH']); packet=normalizeEnterpriseEntityRecord(body);
    }
    const result=await persistPackage(caseId,user.id,packet);
    return NextResponse.json({ok:true,contract:packet.contract,...result,boundary:'Enterprise intake is service-profile routed, persists records and declared relations only after referenced lineage/entities resolve, and does not infer cause, contractual breach, tender winner, supplier rank, or SFI institutional truth.'},{status:201});
