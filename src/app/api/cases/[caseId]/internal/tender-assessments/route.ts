...
import { assertCaseServiceProfileAllowed, assertTenderAssessmentPrerequisites } from '@/lib/sfi/case-platform/integrity';
...
export async function POST(request:Request,context:RouteContext){
  try{
    const{user}=await requireSfiMember(); const{caseId}=await context.params; await assertCaseServiceProfileAllowed(caseId,user.id,['TENDER_ASSURANCE','CUSTOM_RESEARCH']); const body=schema.parse(await request.json());
    ...
    const assessment=normalizeTenderAssessment(body as Parameters<typeof normalizeTenderAssessment>[0]); const object=await recordOperationalCaseObject({caseId,userId:user.id,...assessment});
