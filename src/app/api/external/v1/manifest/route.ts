...
export async function GET() {
  return NextResponse.json({
    ...
    operations: [
      ...
      { id: 'authenticated-governed-machine', method: 'POST', path: '/api/mcp/authenticated', scope: 'per-tool OAuth scope', tenant: 'institutional-user-bound-oauth', contract: 'SFI-AUTHENTICATED-GOVERNED-MACHINE-ADAPTER-1.0 + SFI-AUTHENTICATED-GATEWAY-PROJECTION-1.0', grantContract: 'SFI-CAPABILITY-GRANT-1.0 for invoke_cognitive_capability', grantProofHeader: 'X-SFI-Capability-Grant-Nonce (invoke_cognitive_capability only; transient server-side proof)', description: 'Authenticated MCP projection. Cognitive execution remains ACTIVE-grant + possession-proof bound. The ROOT gateway tool only invokes a closed allowlist of existing canonical OAuth routes and cannot bypass their authority, tenant, queue, evidence, RETURN or sovereign-decision checks.' },
      ...
      { id: 'root-governance-decision', method: 'POST', path: '/governance/proposals/{id}/decision', scope: 'governance:decide', tenant: 'institutional-root-user-bound-oauth', description: 'Accept or deny one proposal through the sovereign ROOT route. Requires root_delegate OAuth plus a currently sovereign ROOT profile; acceptance may queue and dispatch only through the existing governed execution router.' },
      { id: 'root-operate', method: 'POST', path: '/root/operate', scope: 'root:operate', tenant: 'institutional-root-user-bound-oauth', body: { operation: 'capabilities | reports | accounts_list | account_invite' }, description: 'Founder-only bounded ROOT administration for report retrieval and institutional account invitation/listing. This scope does not replace governance:decide, bypass capability grants, mint canon, or grant sovereign authority to invited accounts.' },
      ...
      { id: 'lab-report', method: 'POST', path: '/lab', scope: 'lab:read', tenant: 'institutional', body: { operation: 'report', selectors: { commandId: 'Exact persisted receipt, including source/content/metadata/lineage.', objectId: 'Research object projection and publication package.' } }, description: 'Read institutional Method Lab analyses/evaluations or an exact persisted receipt.' },
      { id: 'lab-persist', method: 'POST', path: '/lab', scope: 'lab:write', tenant: 'institutional', body: { operation: 'persist', required: ['title', 'content'], types: { title: 'nonempty string', content: 'nonempty string' }, commandId: 'Optional idempotency key; reread with report(commandId).', metadata: { kind: 'METHOD_LAB_RESEARCH_OBJECT', researchObject: 'Structured research object; preserve external origin and conduct time separately from registration time.' }, compatibility: 'Legacy researchMetadata is normalized to metadata. Supply only one; malformed metadata is rejected.' }, description: 'Persist a source-derived institutional laboratory record without automatic canonical promotion.' },
      { id: 'lab-run', method: 'POST', path: '/lab', scope: 'lab:run', tenant: 'institutional', body: {
        operation: 'run', required: ['protocolId', 'evidenceIds'],
        protocolIds: ['sociotechnical_simulation', 'economic_simulation'],
        evidenceIds: 'Nonempty array of persisted root_evidence_entries or sfi_evidence_ledger IDs. Epistemic event IDs are not interchangeable with evidence row IDs.',
        optional: { parameters: 'object', cognitiveSpineContextRefs: 'string[]; context only, not evidence' },
        example: { operation: 'run', protocolId: 'economic_simulation', evidenceIds: ['<persisted-evidence-row-id>'], parameters: {} },
      }, output: { result: 'labAnalysisId and run envelope with labRunId, resultHash, epistemicClass=SIMULATED', trace: 'Operational event receipt, or explicit trace persistence error' },
      idempotency: 'NOT_IDEMPOTENT: each accepted request creates a new run. commandId is a persist-only key; do not blindly retry an uncertain run.',
      epistemicClass: 'SIMULATED', canonicalPromotionAllowed: false,
      description: 'Execute one of the two supported simulation protocols with existing lab:run authority. Other registered protocols are not runnable through this operation. Routine internal experimentation requires no second human confirmation.' },
    ...
    machineInterfaces: {
      ...
      authenticated: {
        ...
        gatewayProjection: { contract: 'SFI-AUTHENTICATED-GATEWAY-PROJECTION-1.0', tool: 'invoke_sfi_gateway_operation', canonicalGatewayReused: true, arbitraryUrlAllowed: false, cognitiveGrantBypassAllowed: false, externalEffectsOnlyThroughExistingGatewayAuthorization: true, canonicalPromotionAllowed: false },
