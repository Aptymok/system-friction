...
import { analyzeStructuredStudioObject } from '@/lib/studio/multimodal/structuredAnalyzer';
...
export async function POST(request: Request, ctx: RouteContext) {
  ...
  try {
    ...
    const result = await analyzeStudioModalityObject(objectId, {
      expectedModalities: ['community', 'time_coordinate'],
      force: body.force === true,
      requestedByUserId: context.user.id,
      analyze: ({ bytes, extension, modality }) => {
        if (modality !== 'community' && modality !== 'time_coordinate') {
          throw new StudioMultimodalError('UNSUPPORTED_FILE_TYPE', 'Structured analyzer received an invalid modality.', 415, { modality });
        }
        return analyzeStructuredStudioObject(bytes, extension, modality);
      },
    });
