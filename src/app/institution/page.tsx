...
import { SfiPublicShell } from '@/components/public/SfiPublicShell';
...
export default function InstitutionPage() {
  ...
  return <SfiPublicShell active="INSTITUTE">
    <main className="institutionPage">
      ...
      <section className="institutionSection">
        ...
        <div className="institutionLifecycle">
          {p.lifecycle.map((item, index) => <span key={item}>{String(index + 1).padStart(2, '0')} · {item}</span>)}
        </div>
      </section>
      ...
      <section className="institutionSection">
        ...
        <div className="institutionBoundary">
          {p.invariants.map((item, index) => <article key={item}><h3>{String(index + 1).padStart(2, '0')}</h3><p>{item}</p></article>)}
        </div>
      </section>
      ...
      <section className="institutionSection">
        ...
        <div className="institutionLinks">
          {p.publicSurfaces.map((surface) => <Link key={surface.path} href={surface.path}>
