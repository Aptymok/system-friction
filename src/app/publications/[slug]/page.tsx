...
export default async function PublicationLandingPage({ params }: PageProps) {
  ...
  const temporalSource = publication?.slug === SFI_NOTAS_TEMPORALES_SEPTEMBER_2026_SOURCE.slug
    ? SFI_NOTAS_TEMPORALES_SEPTEMBER_2026_SOURCE
    : null;
    ...
    {publication ? <section style={{ background: '#0d0d09', color: '#d8c6a0', padding: '64px 28px 96px', fontFamily: 'Georgia, serif' }}>
      <article style={{ maxWidth: 980, margin: '0 auto' }}>
        <header style={{ borderBottom: '1px solid rgba(202,160,92,.25)', paddingBottom: 32 }}>
          ...
          <h2 style={{ fontSize: 'clamp(38px,5vw,68px)', fontWeight: 400, margin: '16px 0 10px', color: '#ead5aa' }}>{temporalSource?.subtitle ?? publication.subtitle}</h2>
          <p style={{ fontSize: 21, lineHeight: 1.7, color: '#c9b993', maxWidth: 860 }}>{temporalSource?.coverStatement ?? publication.deck}</p>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center', marginTop: 26, fontFamily: 'Inter,ui-sans-serif,system-ui,sans-serif' }}>
            ...
            {temporalSource?.sourceOfRecord.publicUrl ? <a href={temporalSource.sourceOfRecord.publicUrl} style={{ fontSize: 10, letterSpacing: '.13em', color: '#d5ad69', textDecoration: 'none' }}>ABRIR PDF ↗</a> : null}
          </div>
        </header>

        {temporalSource ? <section id="pdf-edicion" style={{ paddingTop: 52 }}>
          ...
          <p style={{ fontSize: 18, lineHeight: 1.85, color: '#bdb09a' }}>{temporalSource.transportBoundary}</p>
          <dl style={{ border: '1px solid rgba(202,160,92,.22)', padding: 22, display: 'grid', gap: 12, fontFamily: 'Inter,ui-sans-serif,system-ui,sans-serif' }}>
            <div><dt style={{ color: '#9f845b', fontSize: 10, letterSpacing: '.12em' }}>ARCHIVO</dt><dd style={{ margin: '4px 0 0', color: '#d8c6a0' }}>{temporalSource.sourceOfRecord.filename}</dd></div>
            <div><dt style={{ color: '#9f845b', fontSize: 10, letterSpacing: '.12em' }}>TAMAÑO / PÁGINAS</dt><dd style={{ margin: '4px 0 0', color: '#d8c6a0' }}>{temporalSource.sourceOfRecord.byteLength.toLocaleString('es-MX')} bytes · {temporalSource.sourceOfRecord.pages} páginas</dd></div>
            <div><dt style={{ color: '#9f845b', fontSize: 10, letterSpacing: '.12em' }}>SHA-256</dt><dd style={{ margin: '4px 0 0', color: '#d8c6a0', overflowWrap: 'anywhere' }}>{temporalSource.sourceOfRecord.sha256}</dd></div>
            <div><dt style={{ color: '#9f845b', fontSize: 10, letterSpacing: '.12em' }}>PROCEDENCIA</dt><dd style={{ margin: '4px 0 0', color: '#d8c6a0' }}>{temporalSource.sourceOfRecord.provenance}</dd></div>
            <div><dt style={{ color: '#9f845b', fontSize: 10, letterSpacing: '.12em' }}>BINARIO PÚBLICO</dt><dd style={{ margin: '4px 0 0', color: temporalSource.sourceOfRecord.publicUrl ? '#d5ad69' : '#8e806a' }}>{temporalSource.sourceOfRecord.publicUrl ? 'PUBLIC' : 'PENDIENTE DE HOST PÚBLICO CONTROLADO'}</dd></div>
          </dl>
          ...
          <ol style={{ paddingLeft: 22, color: '#bdb09a', lineHeight: 1.7 }}>
            {temporalSource.contents.map((item) => <li key={item.page} style={{ marginBottom: 10 }}><strong style={{ color: '#d5ad69' }}>P. {String(item.page).padStart(2, '0')}</strong> · {item.label} · {item.title}</li>)}
          </ol>
          <p style={{ borderTop: '1px solid rgba(202,160,92,.16)', paddingTop: 18, color: '#a99c87', lineHeight: 1.7 }}><strong style={{ color: '#d5ad69' }}>PRÓXIMA VENTANA:</strong> {temporalSource.nextWindow}</p>
        </section> : null}
