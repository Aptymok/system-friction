...
export const metadata: Metadata = {
  ...
  openGraph: { type: 'website', url: CANONICAL_URL, siteName: 'System Friction Institute', title: 'Publicaciones · System Friction Institute', description: 'Una superficie editorial para observar, contrastar y retornar sin mezclar estados epistemológicos.', images: [{ url: SFI_PUBLICATIONS_BANNER.web }] },
  other: { 'sfi-surface': 'PUBLICATIONS_HUB', 'sfi-editorial-contract': 'SFI-EDITORIAL-FAMILY-PROJECTION-1.0', 'sfi-identity-manual': 'SFI-ID-003 / MASTER EDITION V4.0' },
};
...
export default function PublicationsPage() {
  ...
  return <main className="publicationsHub">
    <header className="pubTopbar">
      ...
      <nav aria-label="Public navigation"><Link href="/observatory">OBSERVATORY</Link><Link href="/publications">PUBLICATIONS</Link><Link href="/library">LIBRARY</Link><Link href="/field">FIELD</Link><Link href="/institution">INSTITUTE</Link><Link href="/login">SIGN IN</Link></nav>
    </header>

    <section className="pubHero" aria-labelledby="publications-title">
      <figure className="pubBanner"><picture><source media="(max-width: 720px)" srcSet={SFI_PUBLICATIONS_BANNER.mobile} /><img src={SFI_PUBLICATIONS_BANNER.web} alt={SFI_PUBLICATIONS_BANNER.alt} /></picture><figcaption>{SFI_PUBLICATIONS_BANNER.provenance}</figcaption></figure>
      <div className="pubHeroCopy"><span>PUBLICACIONES · ARCHIVO EDITORIAL</span><h1 id="publications-title">Observar antes de concluir.</h1><p>SFI publica objetos por función, no por formato. Notas, investigación, métodos, casos y retornos mantienen identidad, versión, procedencia y frontera epistémica aunque su rendition cambie.</p><div className="pubHeroBoundary"><b>REGLA</b><span>PUBLICACIÓN = EXPOSURE</span><span>EXPOSURE ≠ EVIDENCIA EXTERNA ≠ RETURN</span></div></div>
    </section>
    ...
    <section className="temporalIssue" aria-labelledby="temporal-title">
      ...
      <div className="temporalIssueGrid"><figure className="temporalCover">{monthly.coverImage ? <img src={monthly.coverImage} alt={`${monthly.title} · ${monthly.issue}`} /> : null}<figcaption>RENDITION EDITORIAL · identidad canónica preservada</figcaption></figure><article><span className="temporalState">MONTHLY ISSUE · {monthly.issue.toUpperCase()}</span><h2 id="temporal-title">{monthly.title}</h2><h3>{monthly.subtitle}</h3><p>{monthly.deck}</p><dl><div><dt>CADENCIA</dt><dd>Una edición institucional por mes</dd></div><div><dt>ESTADO</dt><dd>As-of state / retorno abierto</dd></div><div><dt>FUNCIÓN</dt><dd>Qué cambió · qué persiste · qué expiró · qué sigue</dd></div></dl><div className="pubActions"><Link href={`/publications/${monthly.slug}`}>ABRIR EDICIÓN →</Link><Link href="/library#notas-temporales">VER ARCHIVO MENSUAL</Link></div></article></div>
    </section>

    <section className="temporalIssue" aria-labelledby="discovery-method-title">
      <div className="sectionKicker"><span>02</span><b>NOTA DE LABORATORIO · DISCOVERY MESH</b><i>SFI-PUB-OBS-014</i></div>
      <div className="temporalIssueGrid"><figure className="temporalCover"><img src="/images/editorial/discovery-mesh-observation.svg" alt="Discovery Mesh · Exposure a Return"/><figcaption>INSTRUMENTO VISUAL · los estados posibles no implican que hayan sido observados</figcaption></figure><article><span className="temporalState">METHOD NOTE · 16 SEP 2026</span><h2 id="discovery-method-title">Discovery Mesh: publicar no es ser encontrado</h2><h3>Cómo SFI observa su propia encontrabilidad sin convertir exposición en reconocimiento.</h3><p>La nota documenta la trayectoria EXPOSURE → DISCOVERY → RECOGNITION → INTERACTION → RELATION → PROPAGATION → PULL → RETURN, las métricas del Mesh y la nueva capacidad de transformar degradaciones observadas en propuestas gobernadas de desarrollo.</p><dl><div><dt>AUTORIDAD</dt><dd>Publicación autorizada por el Fundador</dd></div><div><dt>AUTONOMÍA</dt><dd>El Mesh observa y propone; no se autoaprueba</dd></div><div><dt>FRONTERA</dt><dd>NULL ≠ 0 · propuesta ≠ aprobación · exposure ≠ return</dd></div></dl><div className="pubActions"><Link href={DISCOVERY_NOTE}>LEER NOTA →</Link><Link href="/root/discovery">ABRIR DISCOVERY ROOT</Link></div></article></div>
    </section>

    <section className="observationFamilies" aria-labelledby="families-title">
      <div className="sectionKicker"><span>03</span><b>OBSERVACIONES INDIVIDUALES</b><i>EVENT / EDITORIAL LENSES</i></div>
      <header className="familiesIntro"><h2 id="families-title">Cinco lentes. Un mismo archivo.</h2><p>Las familias organizan lectura y descubrimiento. No reemplazan la clasificación canónica ni convierten una observación en evidencia por presentarla editorialmente.</p></header>
      <div className="familyStack">{SFI_EDITORIAL_FAMILIES.map((family,index) => { const entries = editorialFamilyEntries(family); return <details className="family" key={family.key} open={index === 0}><summary><figure><img src={family.image} alt={family.label} /></figure><div className="familySummaryCopy"><span>{String(index + 1).padStart(2,'0')} · {family.shortLabel.toUpperCase()}</span><h3>{family.label}</h3><p>{family.description}</p><small>{entries.length} {entries.length === 1 ? 'pieza' : 'piezas'} · abrir archivo</small></div><b className="familyToggle" aria-hidden="true">＋</b></summary><div className="familyBody"><p className="imageProvenance">{family.imageProvenance}</p><div className="familyEntries">{entries.map((publication) => <article key={publication.slug}><div className="entryMeta"><span>{publication.observationKind ?? 'OBSERVATION'}</span><time>{formatDate(publication.publishedAt)}</time></div><h4><Link href={`/publications/${publication.slug}`}>{publication.title}</Link></h4><p>{publication.subtitle}</p><footer><Link href={`/publications/${publication.slug}`}>LEER EN SFI →</Link>{publication.mediumUrl ? <a href={publication.mediumUrl} target="_blank" rel="noreferrer">MEDIUM ↗</a> : <span>SFI ORIGINAL</span>}</footer></article>)}</div></div></details>; })}</div>
    </section>
