...
function requireText(value: string, field: string) {
  ...
  if (!normalized) throw new Error(`SFI_SYSTEM_AI_INVALID:${field}`);
  return normalized;
}

function dateOrNull(value: string | null | undefined, field: string) {
  const normalized = value?.trim() || null;
  if (normalized && Number.isNaN(Date.parse(normalized))) throw new Error(`SFI_SYSTEM_AI_INVALID:${field}`);
  return normalized;
}

function ratioOrNull(value: number | null | undefined, field: string) {
  if (value === null || typeof value === 'undefined') return null;
  if (!Number.isFinite(value) || value < 0 || value > 1) throw new Error(`SFI_SYSTEM_AI_INVALID:${field}`);
  return value;
}
...
function uniqueCanonicalRefs(refs: SfiCanonicalRef[]) {
...
function relation(input:Omit<SfiSystemAiRelationDraft,'epistemicRole'> & {epistemicRole?:SfiSystemAiRelationEpistemicRole}):SfiSystemAiRelationDraft {
  const draft:SfiSystemAiRelationDraft={...input,epistemicRole:input.epistemicRole??'RECORD'};
  const violations=validateSystemAiRelationDraft(draft);
...
export function normalizeAiExecutionTrace(input:{executionId:string;aiSystemRef:SfiSystemAiEntityRef;modelRef:SfiSystemAiEntityRef;startedAt:string;finishedAt?:string|null;status:string;promptTemplateRef?:SfiSystemAiEntityRef|null;dataSourceRefs?:SfiSystemAiEntityRef[];toolRefs?:SfiSystemAiEntityRef[];inputHash?:string|null;contextHash?:string|null;outputHash?:string|null;sourceRefs?:SfiCanonicalRef[]}):SfiSystemAiIntakePackage {
  if(input.aiSystemRef.entityType!=='AI_SYSTEM') throw new Error('SFI_AI_EXECUTION_SYSTEM_REF_INVALID');
  if(input.modelRef.entityType!=='AI_MODEL') throw new Error('SFI_AI_EXECUTION_MODEL_REF_INVALID');
  ...
  if(input.promptTemplateRef){
    if(input.promptTemplateRef.entityType!=='PROMPT_TEMPLATE') throw new Error('SFI_AI_EXECUTION_PROMPT_REF_INVALID');
  ...
  for(const ref of input.dataSourceRefs??[]){
    if(ref.entityType!=='DATA_SOURCE') throw new Error('SFI_AI_EXECUTION_DATA_REF_INVALID');
    relations.push(relation({relationKey:`${executionRef.id}:data:${ref.id}`,relationType:'AI_EXECUTION_USES_DATA_SOURCE',from:executionRef,to:ref,sourceRefs:input.sourceRefs,observedAt:startedAt}));
  }
  for(const ref of input.toolRefs??[]){
    if(ref.entityType!=='TOOL') throw new Error('SFI_AI_EXECUTION_TOOL_REF_INVALID');
    relations.push(relation({relationKey:`${executionRef.id}:tool:${ref.id}`,relationType:'AI_EXECUTION_USES_TOOL',from:executionRef,to:ref,sourceRefs:input.sourceRefs,observedAt:startedAt}));
  }
  return {
    contract:SFI_SYSTEM_AI_ASSURANCE_DOMAIN_CONTRACT,
    object:{
...
export function normalizeSystemFailureEvent(input:{failureId:string;occurredAt:string;failureType:string;description?:string|null;componentRef?:SfiSystemAiEntityRef|null;processRef?:SfiSystemAiEntityRef|null;aiExecutionRef?:SfiSystemAiEntityRef|null;sourceRefs?:SfiCanonicalRef[]}):SfiSystemAiIntakePackage {
  const failureRef=systemAiEntityRef('FAILURE_EVENT',input.failureId);
  const occurredAt=dateOrNull(input.occurredAt,'occurredAt');
  if(!occurredAt) throw new Error('SFI_SYSTEM_AI_INVALID:occurredAt');
  const relations:SfiSystemAiRelationDraft[]=[];
  if(input.componentRef){
    if(input.componentRef.entityType!=='COMPONENT') throw new Error('SFI_FAILURE_COMPONENT_REF_INVALID');
    relations.push(relation({relationKey:`${failureRef.id}:component`,relationType:'FAILURE_EVENT_OCCURS_AT_COMPONENT',from:failureRef,to:input.componentRef,sourceRefs:input.sourceRefs,observedAt:occurredAt}));
  }
  if(input.processRef){
    if(input.processRef.entityType!=='PROCESS') throw new Error('SFI_FAILURE_PROCESS_REF_INVALID');
    relations.push(relation({relationKey:`${failureRef.id}:process`,relationType:'FAILURE_EVENT_AFFECTS_PROCESS',from:failureRef,to:input.processRef,sourceRefs:input.sourceRefs,observedAt:occurredAt}));
  }
  if(input.aiExecutionRef){
    if(input.aiExecutionRef.entityType!=='AI_EXECUTION') throw new Error('SFI_FAILURE_EXECUTION_REF_INVALID');
    relations.push(relation({relationKey:`${failureRef.id}:execution`,relationType:'FAILURE_EVENT_ASSOCIATED_WITH_AI_EXECUTION',from:failureRef,to:input.aiExecutionRef,sourceRefs:input.sourceRefs,observedAt:occurredAt}));
  }
  return {
    contract:SFI_SYSTEM_AI_ASSURANCE_DOMAIN_CONTRACT,
...
export function normalizeAiDecisionTrace(input:{decisionId:string;decidedAt:string;disposition:string;aiExecutionRef?:SfiSystemAiEntityRef|null;humanGateRef?:SfiSystemAiEntityRef|null;authorityActorRef?:SfiSystemAiEntityRef|null;sourceRefs?:SfiCanonicalRef[]}):SfiSystemAiIntakePackage {
  const decisionRef=systemAiEntityRef('DECISION_POINT',input.decisionId);
  const decidedAt=dateOrNull(input.decidedAt,'decidedAt');
  if(!decidedAt) throw new Error('SFI_SYSTEM_AI_INVALID:decidedAt');
  const relations:SfiSystemAiRelationDraft[]=[];
  if(input.aiExecutionRef){
    if(input.aiExecutionRef.entityType!=='AI_EXECUTION') throw new Error('SFI_AI_DECISION_EXECUTION_REF_INVALID');
    relations.push(relation({relationKey:`${input.aiExecutionRef.id}:decision:${decisionRef.id}`,relationType:'AI_EXECUTION_PRODUCES_DECISION_INPUT',from:input.aiExecutionRef,to:decisionRef,sourceRefs:input.sourceRefs,observedAt:decidedAt}));
  }
  if(input.humanGateRef){
    if(input.humanGateRef.entityType!=='HUMAN_GATE') throw new Error('SFI_AI_DECISION_GATE_REF_INVALID');
  ...
  if(input.authorityActorRef){
    if(input.authorityActorRef.entityType!=='ACTOR') throw new Error('SFI_AI_DECISION_ACTOR_REF_INVALID');
    relations.push(relation({relationKey:`${input.authorityActorRef.id}:decision:${decisionRef.id}`,relationType:'ACTOR_AUTHORIZES_DECISION',from:input.authorityActorRef,to:decisionRef,sourceRefs:input.sourceRefs,observedAt:decidedAt}));
  }
  return {
    contract:SFI_SYSTEM_AI_ASSURANCE_DOMAIN_CONTRACT,
...
export function buildSystemFrictionAssessment(input:{assessmentId:string;locationRef:SfiSystemAiEntityRef;affectedRefs?:SfiSystemAiEntityRef[];frictionType:string;evidenceRefs:SfiCanonicalRef[];recordRefs?:SfiCanonicalRef[];confidence?:number|null;magnitudeProxy?:number|null}):SfiSystemAiCaseObjectInput {
  if(!input.evidenceRefs.length) throw new Error('SFI_SYSTEM_FRICTION_REQUIRES_EVIDENCE');
  const confidence=ratioOrNull(input.confidence,'confidence');
  const magnitude=ratioOrNull(input.magnitudeProxy,'magnitudeProxy');
  return {
...
export function buildAiImplementationFailureAssessment(input:{assessmentId:string;failureRef:SfiSystemAiEntityRef;layer:SfiAiFailureLayer;evidenceRefs:SfiCanonicalRef[];recordRefs?:SfiCanonicalRef[];determinability:'DETERMINED'|'PARTIALLY_DETERMINED'|'UNDETERMINED';confidence?:number|null;competingHypotheses?:string[]}):SfiSystemAiCaseObjectInput {
  if(input.failureRef.entityType!=='FAILURE_EVENT') throw new Error('SFI_AI_FAILURE_REF_INVALID');
  if(!input.evidenceRefs.length) throw new Error('SFI_AI_FAILURE_ASSESSMENT_REQUIRES_EVIDENCE');
  return {
...
export function buildAiAdoptionOpportunityAssessment(input:{assessmentId:string;processRef:SfiSystemAiEntityRef;useCaseRef:SfiSystemAiEntityRef;evidenceRefs:SfiCanonicalRef[];recordRefs?:SfiCanonicalRef[];projectedValue?:number|null;feasibility?:number|null;integrationRisk?:number|null;requiredControls?:string[]}):SfiSystemAiCaseObjectInput {
  if(input.processRef.entityType!=='PROCESS'||input.useCaseRef.entityType!=='USE_CASE') throw new Error('SFI_AI_OPPORTUNITY_REF_INVALID');
  if(!input.evidenceRefs.length) throw new Error('SFI_AI_OPPORTUNITY_REQUIRES_EVIDENCE');
  return {
...
export type SfiAiGovernanceStageRecordRefs = Partial<Record<SfiAiGovernanceStage,SfiCanonicalRef[]>>;

export function buildAiGovernanceTraceAssessment(input:{assessmentId:string;stageRecordRefs:SfiAiGovernanceStageRecordRefs;evidenceRefs:SfiCanonicalRef[];confidence?:number|null}):SfiSystemAiCaseObjectInput {
  if(!input.evidenceRefs.length) throw new Error('SFI_AI_GOVERNANCE_ASSESSMENT_REQUIRES_EVIDENCE');
  const normalizedStageRecordRefs = Object.fromEntries(
    SFI_AI_GOVERNANCE_STAGES.map((stage) => [stage, uniqueCanonicalRefs(input.stageRecordRefs[stage] ?? [])]),
  ) as Record<SfiAiGovernanceStage,SfiCanonicalRef[]>;
  const stagePresence = Object.fromEntries(
    SFI_AI_GOVERNANCE_STAGES.map((stage) => [stage, normalizedStageRecordRefs[stage].length > 0]),
  ) as Record<SfiAiGovernanceStage,boolean>;
  const missingStages=SFI_AI_GOVERNANCE_STAGES.filter(stage=>!stagePresence[stage]);
  const completeness=(SFI_AI_GOVERNANCE_STAGES.length-missingStages.length)/SFI_AI_GOVERNANCE_STAGES.length;
  const recordRefs=uniqueCanonicalRefs(SFI_AI_GOVERNANCE_STAGES.flatMap((stage)=>normalizedStageRecordRefs[stage]));
  return {
    ...
    payload:domainPayload({
      ...
      auditReady:missingStages.length===0,
...
export const SFI_SYSTEM_AI_ASSURANCE_INVARIANTS={
  oneSharedCasePlatform:true,
  rawPromptRequiredInCaseStore:false,
  rawInputRequiredInCaseStore:false,
  aiOutputEqualsDecision:false,
  failureEqualsRootCause:false,
  frictionEqualsCausality:false,
  projectionEqualsObservedReturn:false,
  traceCompletenessEqualsCompliance:false,
