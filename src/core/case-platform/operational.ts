...
/** Canonical Case Platform lifecycle. UI and API consume this single transition map. */
export const SFI_CASE_TRANSITIONS: Readonly<Record<SfiCaseStatus, readonly SfiCaseStatus[]>> = {
  DRAFT: ['OPEN', 'REJECTED'],
  OPEN: ['OBSERVING', 'ANALYZING', 'CLOSED', 'REJECTED'],
  OBSERVING: ['ANALYZING', 'AWAITING_GOVERNANCE', 'CLOSED', 'REJECTED'],
  ANALYZING: ['OBSERVING', 'AWAITING_GOVERNANCE', 'CLOSED', 'REJECTED'],
  AWAITING_GOVERNANCE: ['ANALYZING', 'INTERVENING', 'CLOSED', 'REJECTED'],
  INTERVENING: ['AWAITING_RETURN', 'ANALYZING', 'CLOSED'],
...
export function nextSfiCaseStatuses(from: SfiCaseStatus) {
  return [...(SFI_CASE_TRANSITIONS[from] ?? [])];
}

export function canSfiCaseTransition(from: SfiCaseStatus, to: SfiCaseStatus) {
  return from === to || includes(SFI_CASE_TRANSITIONS[from], to);
}
...
const exactRole: Partial<Record<SfiCaseObjectKind, SfiEpistemicClass>> = {
  SOURCE: 'SOURCE', RECORD: 'RECORD', EVIDENCE: 'EVIDENCE', OBSERVATION: 'RECORD',
  EPISTEMIC_ASSESSMENT: 'EPISTEMIC_ASSESSMENT', HYPOTHESIS: 'INFERENCE', SYSTEM_MODEL: 'INFERENCE',
  ATTRACTOR: 'INFERENCE', INTERVENTION: 'RECORD', RETURN: 'RECORD', REPORT: 'RECORD',
  GOVERNANCE_DECISION: 'GOVERNANCE_DECISION', UNRESOLVED_QUESTION: 'EPISTEMIC_ASSESSMENT', CONTRADICTION: 'EPISTEMIC_ASSESSMENT',
};

const derivedKinds = new Set<SfiCaseObjectKind>([
  'SYSTEM_MODEL','FRICTION','TRAJECTORY','ATTRACTOR','HYPOTHESIS','INSTRUMENT_RUN','ANALYSIS','RECOMMENDATION',
]);

const forbiddenDerivedAuthority = new Set<SfiEpistemicClass>([
  'SOURCE','EVIDENCE','GOVERNANCE_DECISION','TRUTH_CLAIM',
]);
...
export function validateSfiCaseObjectDraft(draft: SfiCaseObjectDraft): string[] {
  ...
  if (requiredRole && draft.epistemicRole !== requiredRole) violations.push(`CASE_OBJECT_ROLE_MISMATCH:${draft.kind}:${requiredRole}`);
  if (derivedKinds.has(draft.kind) && forbiddenDerivedAuthority.has(draft.epistemicRole)) violations.push(`CASE_DERIVED_OBJECT_AUTHORITY_FORBIDDEN:${draft.kind}:${draft.epistemicRole}`);
  ...
  if (draft.kind === 'INSTRUMENT_RUN' && !includes(['COGNITIVE_EXECUTION','SIMULATION','INFERENCE','PROJECTION'] as const, draft.epistemicRole)) violations.push('CASE_INSTRUMENT_OUTPUT_ROLE_INVALID');
  if (draft.kind === 'ANALYSIS' && !includes(['EPISTEMIC_ASSESSMENT','INFERENCE','SIMULATION','PROJECTION'] as const, draft.epistemicRole)) violations.push('CASE_ANALYSIS_ROLE_INVALID');
  return violations;
}
