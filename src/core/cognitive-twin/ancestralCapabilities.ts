...
export const COGNITIVE_TWIN_POLICY={
  ...
  version:'1.2.0',
  autonomous:[
    'observe','extract','calculate','draft','simulate','propose','persist_memory','propose_subject_mutation',
  ] as const,
  founderReserved:[
    'apply_subject_mutation','publish','mutate_canon','change_formula','grant_root_access','transfer_ip','execute_irreversible',
  ] as const,
  invariants:[
    'EVIDENCE_BEFORE_INFERENCE',
    'SIMULATION_IS_NOT_OBSERVATION',
    'MEMORY_IS_NOT_AUTHORITY',
    'LEARNING_DOES_NOT_EXPAND_AUTHORITY',
    'MISSING_REMAINS_MISSING',
    'LINEAGE_IS_PROVENANCE_NOT_INDIVIDUATION',
  ] as const,
};
...
export const COGNITIVE_TWIN_ANCESTRAL_CAPABILITY_MANIFEST:Array<{
  ...
  {
    id:'timeline_builder',status:'MIGRATED',
    currentImplementation:['sfi_cognitive_twin_memory','sfi_cognitive_twin_runs','sfi_cognitive_twin_decisions','buildCognitiveTwinTimeline()'],
    boundary:'Timeline ordering uses persisted timestamps and source identifiers; missing events are not interpolated.',
  },
  {
    id:'meta_observer',status:'MIGRATED',
    currentImplementation:['readInstitutionalReadiness()','buildCognitiveTwinMetaObservation()','integrated institutional cycle'],
    boundary:'Meta-observation reports organ state and discrepancy; it cannot promote the state it observes.',
  },
  {
    id:'policy_engine',status:'REPLACED_EQUIVALENT',
    currentImplementation:['SFI-CTC 1.2.0','ROOT/ACP proposal lifecycle','COGNITIVE_TWIN_POLICY'],
    boundary:'The policy engine may classify permission but cannot grant itself permission.',
  },
  {
  ...
  {
    id:'identity_state_engine',status:'MIGRATED',
    currentImplementation:['CT-A01 lineage','readCognitiveTwinLineageHealth()','journal/snapshots/forks'],
    boundary:'Identity state means longitudinal provenance state. It does not assert consciousness, personhood or individuation.',
  },
  {
    id:'operating_mode_distribution',status:'MIGRATED',
    currentImplementation:['deriveOperatingModeDistribution()','sfi_cognitive_twin_runs'],
    boundary:'Operating mode is a distribution over auditable institutional run roles/statuses, not first-person psychological self-report.',
  },
  {
    id:'causal_trace',status:'MIGRATED',
    currentImplementation:['sfi_operating_cycles','sfi_inference_traces','buildCognitiveTwinCausalTrace()'],
    boundary:'The trace preserves sequence, hypotheses and linked outcomes; sequence alone never proves causality.',
  },
  {
    id:'governed_mutation',status:'MIGRATED',
    currentImplementation:['readCognitiveTwinMutationState()','CT-A01 mutation proposals','ROOT/ACP reserved apply_subject_mutation'],
    boundary:'Mutation is proposal/evaluation until founder-governed application; it cannot expand institutional authority.',
  },
  {
    id:'external_observation_field',status:'ABSORBED',
    currentImplementation:['WorldSpect','World Vector','Observatory','institutional experience bridge'],
    boundary:'External context preserves source/provenance and does not become friction, causality or canon automatically.',
  },
];
...
export type CognitiveTwinTimelineEvent={
  id:string;
  at:string;
  kind:'MEMORY'|'RUN'|'DECISION';
  source:string;
  status:string|null;
  summary:string|null;
  evidenceRefs:string[];
...
export async function buildCognitiveTwinTimeline(limit=120):Promise<{events:CognitiveTwinTimelineEvent[];warnings:string[]}>{
  ...
  const [memory,runs,decisions]=await Promise.all([
    db.from('sfi_cognitive_twin_memory').select('id,memory_key,memory_type,status,content,evidence_refs,source_kind,source_ref,created_at,updated_at').order('created_at',{ascending:false}).limit(each),
  ...
  const warnings=[memory.error?.message,runs.error?.message,decisions.error?.message].filter((value):value is string=>Boolean(value));
  const events:CognitiveTwinTimelineEvent[]=[];
  for(const item of limitedRows(memory.data,each)){
    const at=timestamp(item.created_at);if(!at)continue;const content=row(item.content);
    events.push({id:String(item.id),at,kind:'MEMORY',source:text(item.source_kind)??'memory',status:text(item.status),summary:text(content.summary)??text(content.title)??text(item.memory_key),evidenceRefs:Array.isArray(item.evidence_refs)?item.evidence_refs.map(String):[]});
  }
...
export async function buildCognitiveTwinMetaObservation(){
  ...
  return {
    ...
    runtimeOperational:readiness.runtimeOperational,
    organs:readiness.modules.map(item=>({id:item.id,state:item.state,observed:item.observed,blockers:item.blockers,evidence:item.evidence})),
    discrepancies:readiness.modules.filter(item=>item.blockers.length||(!item.observed&&item.state!=='READY')).map(item=>({id:item.id,state:item.state,blockers:item.blockers})),
    boundary:'This is a read-only meta-observation of persisted institutional state. It does not promote, approve or repair what it observes.',
  };
}
...
export async function buildCognitiveTwinCausalTrace(operatingCycleId:string){
  ...
  return {
    cycle:cycle.data??null,
    inferences:inferences.data??[],
    trajectory:trajectory.data??[],
    warnings,
    boundary:'Chronology + linked evidence + hypotheses form a causal trace candidate. Causal attribution still requires discriminating evidence or experiment.',
  };
}

export async function recordCognitiveTwinFeedback(input:{
  operatingCycleId:string;
  fieldCaseId:string;
  ...
  return recordCognitiveTwinExperience({
    ...
    sourceRef:input.fieldOutcomeId,
    createdBy:input.createdBy,
    evidenceRefs:input.evidenceRefs,
    content:{
      epistemicClass:'OBSERVED_RETURN',
      operatingCycleId:input.operatingCycleId,
      fieldCaseId:input.fieldCaseId,
      fieldOutcomeId:input.fieldOutcomeId,
      returnId:input.returnId,
      expected:input.expected,
      observed:input.observed,
      contrast:input.contrast,
      rule:'Observed return is candidate institutional experience. It is not verification, authority or canon by storage alone.',
    },
  });
}
...
export async function readCognitiveTwinAncestralState(limit=120){
  const [timeline,operatingMode,metaObservation,lineage,mutations]=await Promise.all([
    ...
    buildCognitiveTwinMetaObservation(),
