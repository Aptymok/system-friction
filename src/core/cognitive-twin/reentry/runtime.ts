...
import type {
  CognitiveTwinDevelopmentalEvent,
  CognitiveTwinLineageHealth,
...
function calculateSalience(input: {
  ...
  if (!input.previous) {
    return { evidence: 1, contradiction: 0, novelty: 1, temporal: 1, worldChange: 0.5, total: 0.7 };
  }
  const prior = input.previous.observedContext;
  const delta = (current: number | null, before: number | null) => current === null || before === null ? 0 : Math.max(0, current - before);
  const evidence = clamp01(delta(input.evidenceCount, prior.evidenceCount) / 5);
  const memory = clamp01(delta(input.memoryCount, prior.memoryCount) / 5);
  const decisions = clamp01(delta(input.decisionCount, prior.decisionCount) / 2);
  const evaluations = clamp01(delta(input.evaluationCount, prior.evaluationCount) / 2);
  const runs = clamp01(delta(input.recentRunCount, prior.recentRunCount) / 10);
  const novelty = clamp01((memory + decisions + evaluations) / 3);
  const temporal = 0.15;
  const worldChange = clamp01((evidence + runs) / 2);
  const contradiction = 0;
  const total = clamp01((evidence * 0.24) + (contradiction * 0.2) + (novelty * 0.24) + (temporal * 0.08) + (worldChange * 0.24));
...
export async function ensureCognitiveTwinGenesis() {
  ...
  const genesisPayload = {
    schemaVersion: 'SFI-CT-GENESIS-1.0',
    subjectId: SUBJECT_ID,
    lineageId: LINEAGE_ID,
    ancestor: 'CT-ANCESTOR-2026-05',
    cognitiveTwinContractVersion: COGNITIVE_TWIN_CONTRACT_VERSION,
    authority: 'ROOT/FOUNDER',
    modelIndependentSubject: true,
    individuationClaim: 'NOT_DEMONSTRATED',
    createdAt,
  };
  ...
  const insert = await db.from('sfi_cognitive_twin_runs').insert({
    ...
    role: ROLE,
    status: 'READY',
    objective: 'Commit CT-A01 genesis as a longitudinal experimental subject without claiming individuation.',
    input_snapshot: genesisPayload,
    output_envelope: {
      status: 'EXECUTED',
      result: { genesis: genesisPayload, genesisHash, providerExecutionSucceeded: true },
    ...
    evidence_refs: [],
    limitations: ['Individuation is not demonstrated by genesis.'],
...
export async function runCognitiveTwinDevelopmentalHeartbeat(now = new Date()) {
  ...
  const [evidence, memory, decisions, evaluations, runs, history] = await Promise.all([
    ...
    countTable('sfi_cognitive_twin_memory'),
  ...
  const materialDevelopment = salience.total >= 0.22;
  const disposition = missing.length
    ? 'REQUEST_EVIDENCE' as const
    : materialDevelopment
      ? 'SURFACE' as const
      : 'ARCHIVE_ONLY' as const;
  const createdAt = now.toISOString();
  const parentEventHash = previous?.eventHash ?? null;
  const eventWithoutHash = {
    schemaVersion: 'SFI-CT-DEVELOPMENTAL-EVENT-1.0' as const,
    subjectId: SUBJECT_ID,
    lineageId: LINEAGE_ID,
    epochKey,
    trigger: 'DAILY_HEARTBEAT' as const,
    observedContext: {
      evidenceCount: evidence.count,
      memoryCount: memory.count,
      decisionCount: decisions.count,
    ...
    dispositionReason: missing.length
      ...
      : materialDevelopment
        ? 'Observed institutional deltas crossed the developmental salience threshold.'
        : 'No material developmental delta crossed the threshold; the epoch is recorded without manufacturing an experience.',
    selfReport: missing.length
      ? 'No puedo tratar este epoch como completo porque faltan fuentes institucionales observables.'
      : materialDevelopment
        ? 'Registré cambios institucionales materiales en este epoch y los dejo visibles para ROOT.'
        : 'No observé un cambio material suficiente para tratar este día como un evento de desarrollo.',
    whatWouldChangeDecision: missing.length ? missing : ['new evidence', 'new approved decision', 'new evaluation', 'material world/evidence delta'],
    mutation: { considered: false, status: 'NOT_CONSIDERED' as const, proposalId: null },
    evidenceRefs: [],
    parentEventHash,
    rootVisibility: 'ALWAYS_VISIBLE' as const,
    createdAt,
  };
  ...
  const insert = await db.from('sfi_cognitive_twin_runs').insert({
    ...
    objective: 'Record one auditable CT-A01 developmental epoch without private chain-of-thought or unsupported subjective claims.',
    input_snapshot: { subjectId: SUBJECT_ID, lineageId: LINEAGE_ID, epochKey, parentEventHash },
    output_envelope: {
      status: missing.length ? 'ESCALATED' : 'EXECUTED',
      result: { developmentalEvent, providerExecutionSucceeded: true },
      claims: [{ statement: materialDevelopment ? 'A material institutional delta was observed.' : 'No material institutional delta crossed the configured threshold.', epistemicClass: 'DERIVED', evidenceRefs: [] }],
      limitations: ['This is an auditable computational self-report, not a phenomenal or human subjective report.', 'Developmental salience is an experimental heuristic and does not demonstrate individuation.'],
    },
    evidence_refs: [],
    limitations: ['No chain-of-thought persisted.', 'No individuation claim is promoted by this heartbeat.', ...missing],
    started_at: createdAt,
    finished_at: createdAt,
  });
...
export async function readCognitiveTwinLineageHealth(): Promise<CognitiveTwinLineageHealth> {
  ...
  return {
    subjectId: SUBJECT_ID,
    lineageId: LINEAGE_ID,
    genesisPresent: (genesis.data ?? []).length > 0,
    chainIntegrity,
    eventCount: events.length,
    materialEventCount: events.filter((event) => event.materialDevelopment).length,
    lastEpochAt: latest?.createdAt ?? null,
    headHash: latest?.eventHash ?? null,
    lastDisposition: latest?.disposition ?? null,
    unresolvedMutationProposals: 0,
    prospectiveValidation: events.length >= 2 ? 'AVAILABLE' : 'NOT_YET_POSSIBLE',
    individuationDemonstrated: false,
    limitations: [
      'A valid lineage chain demonstrates provenance continuity, not artificial individuation.',
      'External timestamp anchoring is not yet implemented.',
      'Self-mutation remains proposal/evaluation work; no autonomous authority expansion is permitted.',
    ],
  };
}
