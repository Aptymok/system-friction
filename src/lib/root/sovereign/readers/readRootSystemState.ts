...
export async function readRootSystemState() {
  const [governance, world, events, audits] = await Promise.all([
    bounded('governanceRuntime', () => readGovernanceRuntime(), 3200),
  ...
  return source(
    {
      ...
      worldVector: world.rows[0] ?? null,
      latestEpistemicEvent: events.rows[0] ?? null,
      latestAudit: audits.rows[0] ?? null,
      matrix: [],
    },
    'governanceRuntime + world_vector + epistemic audit',
    [governance.error, world.error, events.error, audits.error],
    observedAt,
    !governance.data && !world.rows.length,
  );
}
