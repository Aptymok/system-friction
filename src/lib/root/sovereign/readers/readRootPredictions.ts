...
export async function readRootPredictions() {
  const attractorReconciliation = await reconcilePredictionAttractors();
  const [models, runs, requests, outcomes, learning, legacy, verifications] = await Promise.all([
    ...
    selectRows({ table: 'sfi_predictive_evidence_requests', select: 'id,run_id,evidence_key,description,reason,source_candidates,auto_collectible,priority,status,fulfilled_evidence,fulfilled_at,created_at,updated_at', order: 'created_at', limit: 60 }),
    selectRows({ table: 'sfi_predictive_outcomes', select: 'id,run_id,return_window,actual_value,outcome_payload,source_type,source_ref,source_quality,intervention_fidelity,observed_at,evaluation_state,error_payload,created_by,created_at', order: 'observed_at', limit: 60 }),
    selectRows({ table: 'sfi_predictive_learning_events', select: 'id,run_id,outcome_id,model_id,learning_state,error_class,error_analysis,parameter_state_before,parameter_delta,parameter_state_after,quality_weight,amv_reflection,rollback_of,created_by,created_at', order: 'created_at', limit: 60 }),
    selectRows({ table: 'sfi_prediction_entries', select: 'id,hypothesis_id,case_id,case_label,prediccion_explicita,probabilidad_estimativa,fenotipo_estimado,evidence_state,estado_observacion,prediction_registered_at,created_at,updated_at', order: 'created_at', limit: 60 }),
  ...
  return source(
    ...
    [models.error, runs.error, requests.error, outcomes.error, learning.error, legacy.error, verifications.error, ...attractorReconciliation.warnings],
    observedAt,
