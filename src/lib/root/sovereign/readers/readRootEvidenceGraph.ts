...
export async function readRootEvidenceGraph() {
  const [rootEntries, ledger, graphNodes, graphEdges] = await Promise.all([
  ...
  const nodes = graphNodes.rows.map((item) => nodeFrom(item, 'graph_nodes'))
    .filter((node, index, all) => node.id.trim() && node.label.trim() && all.findIndex((candidate) => candidate.id === node.id) === index);
  const nodeIds = new Set(nodes.map((item) => item.id));
  const edges = graphEdges.rows.map((item) => edgeFrom(item, 'graph_edges'))
    .filter((edge, index, all) => edge.id.trim() && edge.from.trim() && edge.to.trim() && nodeIds.has(edge.from) && nodeIds.has(edge.to) && all.findIndex((candidate) => candidate.id === edge.id) === index);
  ...
  return source(
    ...
    'canonical evidence objects + persisted canonical evidence graph',
    [rootEntries.error, ledger.error, graphNodes.error, graphEdges.error],
    observedAt,
