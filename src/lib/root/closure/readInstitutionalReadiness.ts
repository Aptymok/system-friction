...
type ModuleReadiness = { id: string; label: string; state: InstitutionalReadinessState; implemented: boolean; observed: boolean; evidence: string[]; blockers: string[]; externalGates?: string[]; nextAction: string | null };
...
function stateFrom(implemented: boolean, blockers: string[], observed: boolean): InstitutionalReadinessState { if(!implemented)return 'GATED'; if(blockers.length)return 'DEGRADED'; return observed?'OPERATIONAL':'READY'; }
...
export async function readInstitutionalReadiness() {
  ...
  const modules:ModuleReadiness[]=[
    {id:'governance',label:'ROOT / ACP',state:stateFrom(true,governanceBlockers,governance.runtime.status==='active'),implemented:true,observed:governance.runtime.sourceState==='observed',evidence:[`acp:${governance.runtime.status}`,`promotion_receipts:${governance.receipts.promotions}`],blockers:governanceBlockers,nextAction:governance.crl.persistenceDecision==='PENDING_ROOT_ACP_DECISION'?'Create and decide CRL persistence governance proposal.':null},
  ...
  return {generatedAt:new Date().toISOString(),mode:'TOTAL_DEVELOPMENT_CLOSURE',structuralComplete,runtimeOperational,modules,blockers,externalGates,definition:{structuralComplete:'All planned core institutional organs have executable implementations and observable surfaces.',runtimeOperational:'No core organ is DEGRADED/GATED by a missing internal dependency; READY is allowed when implementation is sound but no real run exists yet.',scientificComplete:false,scientificBoundary:'Scientific completion requires preregistered experiments, observed returns, external replication/publication and cannot be achieved by software completion alone.',externalGateBoundary:'Independent publication, timestamping, external data availability and replication are tracked separately from internal software completeness.'}};
}
