...
type Step = { id:string; label:string; status:'PASS'|'BLOCKED'; ref:string|null; detail:string };
...
export async function runFullCycleVerification(input:{ ownerId:string; operatingCycleId:string }) {
  ...
  if(evidence&&event) steps.push({id:'evidence',label:'Evidencia + procedencia',status:'PASS',ref:text(evidence.id),detail:`${text(evidence.title)||'Evidencia'} ↔ ${text(event.event_id)}`});
  else steps.push({id:'evidence',label:'Evidencia + procedencia',status:'BLOCKED',ref:null,detail:'No existe evidencia ROOT reciente cuyo evento epistemológico pueda recuperarse por event_id.'});

  // 2. Real Studio object. Execute the production master loop; provider/runtime failures remain visible.
  ...
  if(studioResult?.ok&&studioRef) steps.push({id:'studio',label:'Studio',status:'PASS',ref:studioRef,detail:`${studioResult.passCount} pases · ${studioResult.convergence}`});
  else steps.push({id:'studio',label:'Studio',status:'BLOCKED',ref:null,detail:text(studioResult?.error)||'No existe objeto Studio ejecutable del propietario o la síntesis real está bloqueada.'});
  ...
  if(labResult?.ok) steps.push({id:'method_lab',label:'Method Lab',status:'PASS',ref:text(labResult.labAnalysisId),detail:`SIMULATED · ${text(labResult.run?.resultHash).slice(0,16)}…`});
  else steps.push({id:'method_lab',label:'Method Lab',status:'BLOCKED',ref:null,detail:text(labResult?.error)||'La prueba de laboratorio no pudo ejecutarse sobre evidencia persistida.'});
  ...
  if(fieldComplete) steps.push({id:'field',label:'Field + retorno',status:'PASS',ref:text(fieldCase?.id),detail:`Retorno ${text(fieldReturn?.status)} · outcome ${text(fieldOutcome?.id).slice(0,12)}…`});
  else steps.push({id:'field',label:'Field + retorno',status:'BLOCKED',ref:null,detail:'No existe un caso Field real con retorno y outcome persistidos para verificar.'});
  ...
  if(twin.ok) steps.push({id:'cognitive_twin',label:'Cognitive Twin',status:'PASS',ref:text(twin.memory?.id),detail:'Replay persistido como experiencia candidata; no promovido.'});
  else steps.push({id:'cognitive_twin',label:'Cognitive Twin',status:'BLOCKED',ref:null,detail:text((twin as any).reason)||'No se pudo persistir la experiencia de verificación.'});
  ...
  if(institutional?.ok) steps.push({id:'institutional_cycle',label:'Ciclo institucional',status:'PASS',ref:text(institutional.run?.id)||text(institutional.taskId),detail:`${institutional.agentCount??0} agentes · CT conectado=${String(institutional.cognitiveTwinIntegration?.connected)}`});
  else steps.push({id:'institutional_cycle',label:'Ciclo institucional',status:'BLOCKED',ref:text(institutional?.run?.id)||null,detail:text(institutional?.error)||arraysToText(institutional?.warnings)||text(institutional?.status)||'El ciclo institucional quedó degradado.'});
  ...
  if(readiness?.runtimeOperational) steps.push({id:'readiness',label:'Readiness',status:'PASS',ref:null,detail:'Todos los órganos núcleo están OPERATIONAL o READY.'});
  else steps.push({id:'readiness',label:'Readiness',status:'BLOCKED',ref:null,detail:text(readiness?.error)||`${readiness?.blockers?.length??0} blocker(s) internos.`});

  const complete=steps.every(step=>step.status==='PASS');
  const finishedAt=new Date().toISOString();
  const proof={contract:'SFI-FULL-CYCLE-PROOF-1.0',mode:'REAL_PERSISTED_EVIDENCE_REPLAY',startedAt,finishedAt,complete,steps,boundary:'No mocks, synthetic outcomes or hardcoded scientific observations are used. Field return is never fabricated or temporally bypassed. A failed dependency remains BLOCKED.'};
