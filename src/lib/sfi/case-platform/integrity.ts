...
import { SFI_SYSTEM_AI_ASSURANCE_DOMAIN_CONTRACT, type SfiEnterpriseEntityRef, type SfiSystemAiEntityRef } from '@/core/case-platform';
...
function text(v:unknown){return typeof v==='string'?v.trim():'';} function object(v:unknown):Record<string,unknown>{return v&&typeof v==='object'&&!Array.isArray(v)?v as Record<string,unknown>:{};} function refId(v:unknown){return text(object(v).id);} function refVersion(v:unknown){return text(object(v).version)||null;} function refHash(v:unknown){return text(object(v).hash)||null;}
...
function matchingRow(rows:Row[],ref:SfiCanonicalRef,predicate:(row:Row)=>boolean){return rows.find(row=>{if(refId(row.canonical_ref)!==ref.id)return false;const expectedVersion=ref.version?.trim()||null;if(expectedVersion&&refVersion(row.canonical_ref)!==expectedVersion)return false;const expectedHash=ref.hash?.trim()||null;if(expectedHash&&refHash(row.canonical_ref)!==expectedHash)return false;return predicate(row);});}
