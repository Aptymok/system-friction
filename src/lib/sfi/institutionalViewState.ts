...
import { buildAttractorScorecard, type AttractorScorecard } from './attractorManagement';
...
export type InstitutionalViewState = {
  ...
  entityContext: { entityId: string; entitySummary: string[]; timeline: Array<{ step: string; value: string }> };
...
export async function readInstitutionalViewState(input?: { entityId?: string; entityType?: string; label?: string }): Promise<InstitutionalViewState> {
  ...
  return {
    metrics: {
      phiSfi: metrics?.phi ?? null, fS: metrics?.fs ?? null, regime: metrics?.regime ?? null, cField, psiMoph: null,
    ...
    entityContext: { entityId, entitySummary: entityContext.entitySummary, timeline: entityContext.timeline },
    ledger,
  };
}
