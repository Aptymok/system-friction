...
export async function bridgeAgentInsightToGovernedProposals(agentId: string, context: KernelContext): Promise<KernelContext> {
  ...
  for (const candidate of interventions) {
    ...
    const created = await createActionProposal({
      ...
      payload: {
        ...
        evidenceRefs,
        authorityBoundary: 'PROPOSAL_REQUIRES_ROOT_GOVERNANCE; THIS BRIDGE DOES_NOT_EXECUTE_INTERVENTION_OR_RECORD_RETURN',
      },
    });
