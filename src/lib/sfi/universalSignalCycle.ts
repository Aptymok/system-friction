...
type HypothesisInput = { statement: string; confidence?: number; role?: 'primary' | 'rival' };
...
export function normalizeUniversalSignal(input: UniversalSignalInput) {
  ...
  const suppliedContentHash = text(input.objectHash)
    ?? text(input.contentHash)
    ?? text(input.fingerprint)
    ?? text(extracted.objectHash)
    ?? text(extracted.contentHash)
    ?? text(extracted.fingerprint)
    ?? text(metadata.objectHash)
    ?? text(provenance.objectHash);
  const referenceIdentity = { contract: SFI_UNIVERSAL_SIGNAL_CONTRACT, kind, name, mimeType, sourceUrl, assetRef };
  const referenceHash = sha256(referenceIdentity);
  const objectHash = suppliedContentHash ?? referenceHash;
  const objectHashBasis = suppliedContentHash ? 'CLIENT_CONTENT_FINGERPRINT' : 'REFERENCE_IDENTITY';
  const normalized = {
    contract: SFI_UNIVERSAL_SIGNAL_CONTRACT,
    kind, name, mimeType, sourceUrl, assetRef,
  ...
  return { ...normalized, objectHash, referenceHash, objectHashBasis, objectKey };
}
...
function methodPlan(input: UniversalCycleInput) {
  ...
  const methods = new Set<string>(['SFI_INFERENCE','DIOL_SF','MIHM_V3','MINIMAL_FIELD_PERTURBATION','OBSERVATION_AND_RESULT_CONTRAST','CONFIGURATION_AND_RESPONSE_LIBRARY']);
...
function agentPlan(input: UniversalCycleInput) {
  ...
  return { totalAvailable: available.length, available, requested: requestedValid.length ? requestedValid : available, note: 'Agents execute roles inside methods; agent output is not epistemic authority and cannot promote itself to canonical state.' };
}
...
export async function readUniversalOpenCycles(limit = 80) {
  ...
  for (const row of universalRows) {
    ...
    const cycleId = text(record(row.payload).cycleId);
  ...
  const universal: OpenUniversalCycle[] = universalRows
    ...
    .map((row): OpenUniversalCycle => ({ ...record(row.payload), eventId: String(row.event_id ?? ''), occurredAt: typeof row.occurred_at === 'string' ? row.occurred_at : null }))
...
export async function readUniversalCycleHistory(cycleId: string) {
  ...
  const returns = events.filter((row) => row.event_name === 'SFI_UNIVERSAL_RETURN_RECORDED');
  ...
  return {
    ok: true,
    cycleId,
    state: closures.length ? 'CLOSED' : returns.length ? 'RETURN_RECORDED' : structuredResults.length ? 'AWAITING_RETURN' : 'OPEN',
    opened,
    cognitiveRuns,
    structuredResults,
    returns,
...
export async function persistUniversalSignal(input: UniversalCycleInput, actorId: string, tenantId: string) {
  ...
  const event = await appendEpistemicEvent({
    eventName: 'SFI_UNIVERSAL_SIGNAL_INGESTED', epistemicClass: 'declared', confidence: 1,
    payload: { actorId, tenantId, signal, question: text(input.question), objective: text(input.objective), declaredFunction: text(input.declaredFunction), systemType: text(input.systemType), context: record(input.context), declaredTarget: input.declaredTarget ?? null, declaredExclusions: Array.isArray(input.declaredExclusions) ? input.declaredExclusions : [], invariants: Array.isArray(input.invariants) ? input.invariants : [], constraints: Array.isArray(input.constraints) ? input.constraints : [], epistemicBoundary: 'The supplied object and operator declarations are persisted as declared input. Extracted, derived, inferred and simulated states remain separate.' },
    occurredAt: new Date().toISOString(), source: { sourceId: actorId, sourceType: 'external_signal_intake' }, logbookId: `signal:${signal.objectHash}`, lineage: [signal.objectHash],
  });
  return { signal, event };
}
...
export async function runUniversalCognitiveCycle(input: UniversalCycleInput, actorId: string, tenantId: string) {
  ...
  context.evidence.push({ id: signal.objectHash, source: 'UniversalSignalGateway', confidence: 1, payload: { epistemicClass: 'declared', signal, actorId, tenantId, question: text(input.question), objective: text(input.objective), declaredFunction: text(input.declaredFunction), context: record(input.context) } });
  ...
  context.metadata = {
    ...
    worldSpect: worldSnapshot ? snapshotRowToApiData(worldSnapshot) : null, worldContextUsed: useWorldContext, requestedAgents: agentPlan(input).requested, llmAugmentation: input.llmAugmentation === true,
  ...
  const opened = await appendEpistemicEvent({
    ...
    payload: { cycleId, taskId, objectKey: signal.objectKey, objectHash: signal.objectHash, referenceHash: signal.referenceHash, objectHashBasis: signal.objectHashBasis, actorId, tenantId, question: text(input.question), objective: text(input.objective), methodPlan: methodPlan(input), agentPlan: agentPlan(input), worldSnapshotId: worldSnapshot?.id ?? null, worldContextUsed: useWorldContext },
    occurredAt: new Date().toISOString(), source: { sourceId: 'universal_signal_gateway', sourceType: 'cognitive_runtime' }, logbookId, lineage: [signal.objectHash],
  });
  ...
  return { signal, cycleId, taskId, logbookId, methodPlan: methodPlan(input), agentPlan: agentPlan(input), worldSnapshot: worldSnapshot ? snapshotRowToApiData(worldSnapshot) : null, result, opened, completedEvent };
}
...
export async function closeUniversalCycle(input: { cycleId: string; objectKey?: string; reason: string; evidenceRefs?: string[] }, actorId: string, tenantId: string) {
  return appendEpistemicEvent({
    eventName: 'SFI_UNIVERSAL_CYCLE_CLOSED', epistemicClass: 'derived', confidence: 1,
    payload: { cycleId: input.cycleId, objectKey: input.objectKey ?? null, actorId, tenantId, reason: input.reason, evidenceRefs: Array.isArray(input.evidenceRefs) ? input.evidenceRefs : [], closureBoundary: 'Closure means the methodological question has sufficient disposition; it does not mean the observed system is permanently resolved.' },
    occurredAt: new Date().toISOString(), source: { sourceId: actorId, sourceType: 'external_cycle_closure' }, logbookId: `universal-cycle:${input.cycleId}`, lineage: Array.isArray(input.evidenceRefs) ? input.evidenceRefs : [],
  });
}

export function describeUniversalSignalContract(input: UniversalCycleInput) {
  return { signal: normalizeUniversalSignal(input.signal), clarifyingQuestions: buildClarifyingQuestions(input), methodPlan: methodPlan(input), agentPlan: agentPlan(input) };
}
