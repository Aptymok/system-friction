...
export async function saveSfiLabAnalysis(
  analysis: SfiLabAnalysis,
) {
  localAnalyses.set(
    analysis.analysisId,
    analysis,
  ...
  try {
    ...
    if (analysis.reappearances.length) {
      await supabase
        .from('sfi_reappearances')
        .insert(
          analysis.reappearances.map(
            (item) => ({
              id: item.id,
              analysis_id:
                analysis.analysisId,
    ...
    return {
      ...
      storage: 'local_memory' as const,
      warning:
        'supabase_persistence_failed',
    }
  }
}

export async function getSfiLabAnalysis(
  analysisId: string,
) {
  const local =
    localAnalyses.get(analysisId)
