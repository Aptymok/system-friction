...
function routeDisposition(): FounderRouteDisposition[] {
  return [
    ['/', 'public landing', 'static page + SFI_NAVIGATION', 'route_disposition', 'keep_static', 'Final public entry route.', 'Keep public.'],
    ['/repository', 'public documentation/evidence repository', 'public repo page', 'atlas', 'keep_static', 'Approved public documents and evidence only.', 'Keep public.'],
    ['/root', 'private founder/root console', '/api/root/founder-state + /api/root/* + safe ACP/mutation endpoints', 'operations', 'keep_as_data_provider', 'Founder SFI-01 control surface.', 'Keep ROOT protected.'],
    ['/field', 'public Mini MOP-H product', 'public field intake + gated User Twin/persistence when account exists', 'field', 'keep_as_data_provider', 'Normal-user operational field with optional account memory.', 'Keep intake public; gate Twin and persistence.'],
    ['/studio', 'private Edwing / REM618 producer field', 'local studio state until approved STUDIO event contract exists', 'field', 'keep_as_data_provider', 'Private producer field, not public.', 'Keep restricted to ROOT/authorized studio account.'],
    ['/contact', 'contact form and links', 'mailto fallback', 'route_disposition', 'keep_static', 'Public contact route.', 'Keep public.'],
    ['/privacy', 'privacy policy', 'static page', 'route_disposition', 'keep_static', 'Required public policy.', 'Keep public.'],
    ['/login', 'sign in', 'loginAction', 'route_disposition', 'keep_static', 'Canonical sign-in route.', 'Keep public auth.'],
    ['/signup', 'create account', 'registerAction', 'route_disposition', 'keep_static', 'Canonical signup route.', 'Keep public auth.'],
    ['/founder-console', 'founder surface/provider', '/api/root/founder-state now also used by /root', 'operations', 'keep_as_data_provider', 'Founder state remains part of ROOT operational assembly.', 'Keep available as provider or route if present.'],
    ['/sfi-console', 'SFI operational source', 'readOperationalConsoleState internal library', 'my_panel', 'keep_as_data_provider', 'Operational console read model feeds ROOT and agentic reports.', 'Keep as canonical SFI source.'],
    ['/world-vector', 'World Vector product surface', '/api/worldspect/* internal providers', 'timeline', 'integrate_into_founder_console', 'World Vector remains an independent route and ROOT panel.', 'Keep route and integrate as ROOT panel.'],
    ['/scorefriction', 'ScoreFriction product surface', '/api/scorefriction/* internal providers', 'evaluation', 'integrate_into_founder_console', 'ScoreFriction remains an evidence and operational provider.', 'Keep route and provider APIs.'],
    ['/moph', 'MOP-H source surface', 'MOP-H local field logic + /api/moph/session', 'field', 'integrate_into_founder_console', 'Mini MOP-H now lives in /field while original contracts remain providers.', 'Keep contracts and route if present.'],
    ['/observatory', 'observatory shell/provider', 'field components reused by current product contracts', 'field', 'keep_as_data_provider', 'Observatory modules remain evidence providers.', 'Keep available as provider or route if present.'],
    ['/campo', 'field source material', 'field source material', 'field', 'keep_as_data_provider', 'Field variants remain source material until explicitly migrated.', 'Keep available as provider or route if present.'],
    ['/surfaces', 'surface index/provider', 'route disposition inside /root', 'route_disposition', 'keep_as_data_provider', 'Surface knowledge remains useful for route governance.', 'Keep available as provider or route if present.'],
    ['/instruments', 'instrument index/provider', 'final route map', 'route_disposition', 'keep_as_data_provider', 'Instrument registry remains useful for AMV scope discovery.', 'Keep available as provider or route if present.'],
    ['/observatories', 'AMV scope index/provider', 'internal AMV scope state', 'route_disposition', 'keep_as_data_provider', 'Observatory scopes remain useful for AMV and Graph.', 'Keep available as provider or route if present.'],
    ['/cluster-atlas', 'Atlas source surface', 'derived Atlas in /root and attractor hub in /field', 'atlas', 'integrate_into_founder_console', 'Atlas remains part of ROOT drafting and memory.', 'Keep available as provider or route if present.'],
    ['/signal-vane', 'signal source surface', 'timeline/evidence interpretation in /root and /field', 'timeline', 'integrate_into_founder_console', 'Signal Vane remains a signal provider.', 'Keep available as provider or route if present.'],
    ['/cognitive-twin-engine', 'twin source surface', 'agent diagnostics in /root and studio memory', 'operations', 'integrate_into_founder_console', 'Twin runtime remains part of Cognitive Twin.', 'Keep available as provider or route if present.'],
    ['/governance-reality', 'governance source surface', 'mutation approvals in /root', 'system_mutation', 'integrate_into_founder_console', 'Governance remains approval and mutation context.', 'Keep available as provider or route if present.'],
    ['/terminal', 'terminal source surface', 'internal APIs only', 'operations', 'keep_as_data_provider', 'Terminal contracts remain source material for operational tooling.', 'Keep available as provider or route if present.'],
...
function buildField(input: {
  ...
  const nodes: FounderFieldNode[] = [
    ...
    ['protected', 'Protected Surfaces', 'access boundary', '/root /field /studio', 'blocked', 'route_disposition', 'protect'],
...
export async function buildFounderConsoleState(origin?: string): Promise<FounderConsoleState> {
  ...
  return {
    ...
    myPanel: {
      ...
      showable: ['/', '/repository', '/contact', '/privacy'],
      protected: ['/root', '/studio'],
