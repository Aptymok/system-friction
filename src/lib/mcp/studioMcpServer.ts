...
export function isStudioMcpRequest(value: unknown): value is JsonRpcRequest {
...
export function studioMcpProtocolVersionFor(value: unknown) {
  const requested = typeof row(row(value).params).protocolVersion === 'string'
    ? String(row(row(value).params).protocolVersion).trim()
    : '';
  return LEGACY_MCP_PROTOCOL_VERSIONS.has(requested) ? requested : SFI_STUDIO_MCP_PROTOCOL_VERSION;
}

export function studioMcpRequiredScope(value: unknown) {
  const request = row(value);
  if (request.method !== 'tools/call') return 'studio:read';
  const tool = findTool(row(request.params).name);
  return tool?.requiredScope ?? 'studio:read';
}
...
function statusPayload() {
  return {
    ...
    authorityBoundary: {
      ...
      publicCapabilityCatalogPublished: false,
    },
...
export async function dispatchStudioMcpRequest(
  ...
  if (!isStudioMcpRequest(value)) {
    return { status: 400, body: jsonRpcError(null, -32600, 'Invalid Request', { reason: 'INVALID_JSON_RPC_REQUEST' }) };
  }
  const request = value;
  ...
  if (method === 'initialize') {
    return {
      ...
      body: jsonRpcResult(id, {
        protocolVersion: studioMcpProtocolVersionFor(value),
  ...
  if (method === 'ping') return { status: 200, body: jsonRpcResult(id, {}) };
  ...
  if (method === 'resources/read') {
    ...
    return {
      status: 200,
      body: jsonRpcResult(id, {
        contents: [{ uri: 'sfi://studio/status', mimeType: 'application/json', text: JSON.stringify(payload) }],
      }),
    };
  }
  ...
  if (method === 'tools/call') {
    ...
    const invoked = await dependencies.invokeStudioOperation(tool.operation, row(params.arguments));
    if (invoked.status === 401) {
      return {
        status: 401,
        body: jsonRpcError(id, -32001, 'Unauthorized', { requiredScope: tool.requiredScope, studio: invoked.body }),
      };
    }
    if (invoked.status === 403) {
      return {
        status: 403,
        body: jsonRpcError(id, -32003, 'Forbidden', { requiredScope: tool.requiredScope, studio: invoked.body }),
      };
    }

    const isError = invoked.status < 200 || invoked.status >= 300;
    return { status: 200, body: jsonRpcResult(id, toolResult(invoked.body, isError)) };
  }
