...
export async function reconcilePersistedEvidenceGraph() {
  ...
  const [rootEvidence, ledger] = await Promise.all([
    db.from('root_evidence_entries').select('id,evidence_hash,title,evidence_type,target_node_id,payload,epistemic_event_id,created_at').order('created_at', { ascending: true }).limit(1000),
    db.from('sfi_evidence_ledger').select('id,case_id,module,evidence_kind,source_name,evidence_hash,observed_at,created_at').order('created_at', { ascending: true }).limit(1000),
  ]);

  const warnings = [rootEvidence.error?.message, ledger.error?.message].filter((value): value is string => Boolean(value));
  let nodesCreated = 0; let nodesUpdated = 0; let edgesCreated = 0;

  for (const item of rows(rootEvidence.data)) {
    const id = text(item.id); const hash = text(item.evidence_hash); if (!id || !hash) continue;
    ...
    const targetNodeId = text(item.target_node_id); if (!targetNodeId) continue;
