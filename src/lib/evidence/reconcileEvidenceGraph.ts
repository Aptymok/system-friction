...
function compactWarnings(input: string[]) {
  ...
  for (const warning of unique) {
    const constraint = warning.match(/violates check constraint "([^"]+)"/)?.[1];
  ...
  return [...buckets.entries()].map(([key, value]) => value.count > 1 ? `${key} · ${value.count} reconciliaciones rechazadas · ${value.sample.split(':').slice(-1)[0]}` : value.sample);
}
...
export async function reconcilePersistedEvidenceGraph() {
  ...
  for (const item of rootEvidence.data) {
    ...
    const upsert = await db.from('graph_nodes').upsert({
      node_id: nodeId, node_key: nodeId, label: text(item.title) ?? `Evidence ${id.slice(0, 8)}`, node_type: LEGACY_NODE_STORAGE_TYPE, ontology_type: 'evidence', origin: 'root_evidence', epistemic_class: epistemicClass, confidence: 1, payload: nodePayload, lineage: eventId ? [eventId] : [], attributes: nodePayload, updated_at: new Date().toISOString(),
    }, { onConflict: 'node_id' });
    ...
    if (targetNodeId) {
      ...
      if (resolvedTarget) {
        await upsertEdge({ from: nodeId, to: resolvedTarget, relation: text(payload.relationType) ?? 'contextualizes', relationType: 'structural_declared', confidence: 1, observedAt, evidenceIds: eventId ? [eventId] : [], attributes: { evidenceHash: hash, verified: false, explicitTargetNode: true } });
      }
    }
  }
  ...
  for (const item of ledger.data) {
    ...
    descriptors.push({ id, hash, nodeId, label: text(publicSummary.title) ?? text(item.source_name) ?? text(item.evidence_kind) ?? `Ledger evidence ${id.slice(0, 8)}`, caseId: text(item.case_id), module: text(item.module), evidenceKey: text(publicSummary.evidenceKey), sourceUrl: text(item.source_url), observedAt });
  }
  ...
  for (const [hash, items] of byHash.entries()) {
    ...
    for (const other of rest) {
      await upsertEdge({ from: first.nodeId, to: other.nodeId, relation: 'same_evidence_object', relationType: 'identity_reference', confidence: 1, observedAt: laterDate(first.observedAt, other.observedAt), attributes: { evidenceHash: hash } });
    }
  }
  ...
  for (const descriptor of descriptors) {
    ...
    const surface = await db.from('graph_nodes').upsert({
      node_id: sourceNodeId, node_key: sourceNodeId, label: urlLabel(descriptor.sourceUrl), node_type: LEGACY_NODE_STORAGE_TYPE, ontology_type: 'source_surface', origin: 'evidence_provenance', epistemic_class: 'imported', confidence: 1, payload: surfacePayload, attributes: surfacePayload, lineage: [], updated_at: new Date().toISOString(),
    }, { onConflict: 'node_id' });
    if (surface.error) warnings.push(`source_surface:${descriptor.sourceUrl}:${surface.error.message}`);
    else if (existing.data) nodesUpdated += 1; else nodesCreated += 1;
    await upsertEdge({ from: descriptor.nodeId, to: sourceNodeId, relation: 'declares_source_surface', relationType: 'provenance_declared', confidence: 1, observedAt: descriptor.observedAt, attributes: { sourceUrl: descriptor.sourceUrl } });
  }
  ...
  for (const attractor of rows(attractorRead.data)) {
    ...
    const upsert = await db.from('graph_nodes').upsert({
      node_id: attractorNodeId, node_key: attractorNodeId, label: text(attractor.label) ?? attractorKey, node_type: LEGACY_NODE_STORAGE_TYPE, ontology_type: 'attractor', origin: 'sfi_attractors', epistemic_class: 'declared', confidence: number01(attractor.confidence, 0), payload: attractorPayload, attributes: attractorPayload, lineage: [], updated_at: new Date().toISOString(),
    }, { onConflict: 'node_id' });
