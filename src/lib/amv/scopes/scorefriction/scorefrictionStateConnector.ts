...
function logSupabaseClientDiagnostics() {
  ...
  if (!serviceRolePresent) {
    console.warn('[scorefrictionStateConnector] scorefriction_service_role_missing')
  }

  return { serviceRolePresent }
}

async function queryRows(service: SupabaseClient, table: string, select = '*', order = 'created_at', limit = 25, eq?: { column: string; value: string }) {
...
export async function buildScoreFrictionSelectedContext() {
  ...
  try {
    if (!diagnostics.serviceRolePresent) {
      warnings.push('scorefriction_service_role_missing')
      throw new Error('scorefriction_service_role_missing')
    }
...
export async function buildScoreFrictionScopeState(): Promise<AmvScopeState> {
  ...
  const canFeedRegime = liveEnough && selectedContext.trust === 'observed' && selectedContext.source_coverage > 0
