...
function qualifiesForPromotion(occurrence: OperationOccurrence): boolean {
  return occurrence.epistemicClass === 'OBSERVED' || occurrence.epistemicClass === 'VERIFIED_CONTRAST';
}
...
export function evaluateCounterfactualProbes(probes: CounterfactualProbe[]): CounterfactualEvaluation {
  ...
  return {
    ...
    observedOrVerifiedProbeCount: probes.filter((probe) => probe.epistemicClass === 'OBSERVED' || probe.epistemicClass === 'VERIFIED_CONTRAST').length,
...
export function buildDecisionTransferEvaluation(input: {
  ...
  const nonSimulatedEvidence = input.promotion.qualifyingSupportCount > 0
    && (input.counterfactual.observedOrVerifiedProbeCount > 0 || input.promotion.verifiedContrastCount > 0);
  const pass = input.holdout.decisionAccuracy >= gates.minimumDecisionAccuracy
    && input.holdout.meanStructuralFidelity >= gates.minimumStructuralFidelity
    && input.counterfactual.targetDispositionAccuracy >= gates.minimumCounterfactualTargetAccuracy
  ...
  return {
    ...
    limitations: [
      ...
      'Simulation can exercise the instrument but cannot promote an operation, pattern or rule.',
