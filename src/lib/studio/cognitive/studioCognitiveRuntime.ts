...
function parseFinalResult(raw: string, allowedEvidence: Set<string>): FinalResult | null {
  try {
    ...
    const changes = Array.isArray(parsed.changes) ? parsed.changes.map(record).slice(0, 3).map((item) => ({ label: text(item.label, 240) ?? 'Cambio', before: (typeof item.before === 'number' || typeof item.before === 'string') ? item.before : null, after: (typeof item.after === 'number' || typeof item.after === 'string') ? item.after : null, meaning: text(item.meaning, 800) ?? '' })) : [];
    ...
    return {
      summary: text(parsed.summary), findings, inconsistencies, changes,
      identity: { status: identityStatus, confidence: number(identityRaw.confidence), reason: text(identityRaw.reason, 1200) ?? 'No se proporcionó razón suficiente.', preserved: strings(identityRaw.preserved, 8) },
      production: { status: productionStatus, reason: text(productionRaw.reason, 1200) ?? 'Estado de producción no justificado.', blockers: strings(productionRaw.blockers, 3) },
      ejector: { direction: strings(ejectorRaw.direction, 6), magnitude: typeof ejectorRaw.magnitude === 'number' ? number(ejectorRaw.magnitude) : null, velocity: typeof ejectorRaw.velocity === 'number' ? Number(ejectorRaw.velocity) : null, horizon: text(ejectorRaw.horizon, 160), confidence: number(ejectorRaw.confidence), causalNodes: strings(ejectorRaw.causalNodes, 8), basis: text(ejectorRaw.basis, 1000) ?? 'Insufficient basis.' },
      hypothesis: Object.keys(hypothesisRaw).length ? { statement: text(hypothesisRaw.statement, 1200) ?? '', perturbation: text(hypothesisRaw.perturbation, 1200) ?? '', expectedOutput: text(hypothesisRaw.expectedOutput, 1200) ?? '', controls: strings(hypothesisRaw.controls, 8), evidenceRequired: strings(hypothesisRaw.evidenceRequired, 8), falsificationCriterion: text(hypothesisRaw.falsificationCriterion, 1200) ?? '' } : null,
...
function kernelEvidence(studio: Awaited<ReturnType<typeof readStudioProductionState>>, relational: Awaited<ReturnType<typeof analyzeStudioSessionRelations>>): KernelEvidence[] {
  const evidence: KernelEvidence[] = studio.evidence.slice(0, 80).map((item) => ({ id: item.id, source: item.source, confidence: item.reliability, payload: { type: item.type, label: item.label, observedAt: item.observedAt, uri: item.uri } }));
  studio.metricValues.filter((metric) => metric.value !== null).slice(0, 80).forEach((metric) => evidence.push({ id: `metric:${metric.key}`, source: metric.source ?? 'studio_metric', confidence: metric.confidence, payload: { key: metric.key, label: metric.label, value: metric.value, unit: metric.unit, status: metric.status, explanation: metric.explanation, warnings: metric.warnings, evidenceIds: metric.evidenceIds } }));
...
function synthesisSystem(action: StudioCognitiveAction) {
  return [
    ...
    action === 'verify' ? 'Use the returned evidence to classify the active hypothesis as SUPPORTED, FALSIFIED or INCONCLUSIVE.' : 'hypothesisOutcome must be null.',
...
export async function runStudioCognitiveRuntime(input: { ownerId: string; objectId: string; action: StudioCognitiveAction }) {
  ...
  const relational = await analyzeStudioSessionRelations({ sessionId: studio.session.id, ownerId: input.ownerId, activeObjectId: input.objectId }).catch((error) => ({ engine: 'studio_audio_relational_v1' as const, sessionId: studio.session.id as string, generatedAt: new Date().toISOString(), objectCount: session.objects.length, audioObjectCount: 0, roleAssignments: [], pairwise: [], routeTests: [], stateContrast: null, findings: [], warnings: [`relational_analysis_failed:${error instanceof Error ? error.message : String(error)}`] }));
  ...
  const context: KernelContext = {
    cycleId: randomUUID(), logbookId: `studio:${studio.session.id}:${randomUUID()}`, phenomenonId: input.objectId, taskId: `studio-${input.action}-${randomUUID()}`, currentEvent: `STUDIO_${input.action.toUpperCase()}`,
    evidence,
    hypotheses: session.activeHypothesis ? [{ id: String(session.activeHypothesis.id), statement: text(session.activeHypothesis.statement) ?? 'Active Studio hypothesis', confidence: number(activeHypothesisPayload?.confidence, 0.5) }] : [],
    contradictions: [], simulations: [], predictions: [], risks: [], opportunities: [],
    metadata: {
      studioAction: input.action, requestedAgents: agents, llmAugmentation: true, preferredLlmProvider: 'groq', cognitiveTwinContext: twin,
      studio: {
        sessionId: studio.session.id, objectId: input.objectId, objectTitle: studio.activeObject.title, objectType: studio.activeObject.type,
        analysisStatus: studio.activeObject.analysisStatus, readiness: studio.activeObject.readiness, metrics: studio.metricValues.filter((metric) => metric.value !== null).slice(0, 80),
        culturalLens: studio.culturalLens, mihm: studio.mihmReport, fieldGraph: studio.fieldGraph, relational,
        activeHypothesis: session.activeHypothesis ? { id: String(session.activeHypothesis.id), statement: session.activeHypothesis.statement, payload: session.activeHypothesis.payload, createdAt: session.activeHypothesis.created_at } : null,
  ...
  const synthesisPrompt = JSON.stringify({
    ...
    relational,
    activeHypothesis: session.activeHypothesis ? { id: String(session.activeHypothesis.id), statement: session.activeHypothesis.statement, recommendedChange: session.activeHypothesis.recommended_change, payload: session.activeHypothesis.payload } : null,
    ...
    agentInsights,
    cognitiveTwin: { contractVersion: twin.contractVersion, memory: twin.memory.slice(0, 30), approvedDecisions: twin.decisions.slice(0, 24), warnings: twin.warnings },
  });
  const synthesis = await runLlmTask({ task: 'graph_interpretation', system: synthesisSystem(input.action), prompt: synthesisPrompt, fallbackResult: '{"status":"LLM_UNAVAILABLE"}', preferredProvider: 'groq', maxTokens: 1800 });
  ...
  const finalResult: FinalResult = parsed ?? {
    summary: null, findings: [], inconsistencies: [], changes: [],
    identity: { status: 'INDETERMINATE', confidence: 0, reason: synthesis.ok ? 'El modelo no devolvió el contrato JSON requerido.' : 'No existe un proveedor LLM disponible para cerrar la síntesis.', preserved: [] },
    production: { status: 'EVIDENCE_PENDING', reason: 'Studio no representa una síntesis degradada como análisis final.', blockers: [synthesis.ok ? 'LLM_RESPONSE_SCHEMA_INVALID' : 'LLM_PROVIDER_UNAVAILABLE'] },
    ejector: { direction: [], magnitude: null, velocity: null, horizon: null, confidence: 0, causalNodes: [], basis: 'No calculado.' },
    hypothesis: null, hypothesisOutcome: input.action === 'verify' ? 'INCONCLUSIVE' : null,
  };
  ...
  const envelope = createCognitiveTwinEnvelope({
    taskId: context.taskId!, status: synthesis.ok ? 'EXECUTED' : 'ESCALATED', modelId: synthesis.ok ? `${synthesis.provider}:${synthesis.model}` : null, result: finalResult,
    ...
    limitations: unique([...relational.warnings, ...twin.warnings, ...synthesis.warnings]), contradictions: finalResult.inconsistencies.map((item) => item.statement),
    missingEvidence: finalResult.production.status === 'EVIDENCE_PENDING' ? finalResult.production.blockers : [], actionsExecuted: cycle.executedAgents.map((agent) => `agent:${agent}`),
    testsRun: ['studio_object_analysis', relational.audioObjectCount > 1 ? 'studio_audio_relational_v1' : 'studio_audio_relational_not_applicable'], recommendedTransition: finalResult.production.status === 'READY' ? 'VERIFYING' : 'EVIDENCE_PENDING',
  });
  ...
  if (input.action === 'generate_hypothesis' && finalResult.hypothesis) {
    const inserted = await db.from('studio_hypotheses').insert({
      object_id: input.objectId, owner_id: input.ownerId, origin: 'studio_cognitive_runtime_v1', severity: finalResult.inconsistencies.some((item) => item.severity === 'high') ? 'high' : 'medium',
      statement: finalResult.hypothesis.statement, recommended_change: finalResult.hypothesis.perturbation, route: 'FAD_MINIMUM_PERTURBATION', sources: evidenceRefs.slice(0, 40),
      payload: { suggestionStatus: 'EVIDENCE_PENDING', status: 'EVIDENCE_PENDING', confidence: finalResult.identity.confidence, expectedSignal: finalResult.hypothesis.expectedOutput, controls: finalResult.hypothesis.controls, evidenceRequired: finalResult.hypothesis.evidenceRequired, falsificationCriterion: finalResult.hypothesis.falsificationCriterion, taskId: context.taskId, agentIds: cycle.executedAgents, provider: synthesis.provider, model: synthesis.model },
    }).select('id').single();
  ...
  if (input.action === 'verify' && session.activeHypothesis) {
    ...
    await db.from('studio_hypotheses').update({ payload: { ...record(session.activeHypothesis.payload), suggestionStatus: nextStatus, status: nextStatus, verifiedAt: finishedAt, verificationObjectId: input.objectId, verificationEvidenceIds: returnedEvidenceAfterHypothesis.map((row) => String(row.id)), hypothesisOutcome: outcome } }).eq('id', String(session.activeHypothesis.id)).eq('owner_id', input.ownerId);
    await persistStudioLearningCandidate({
      ...
      memoryType: 'EVIDENCE', content: { hypothesisId: String(session.activeHypothesis.id), hypothesisOutcome: outcome, result: finalResult, relational },
      evidenceRefs: returnedEvidenceAfterHypothesis.map((row) => String(row.id)), sourceRef: input.objectId, createdBy: input.ownerId,
    });
  }

  const trace = await db.from('studio_evidence_traces').insert({
    object_id: input.objectId, owner_id: input.ownerId, source: 'studio_cognitive_runtime_v1', label: `Studio cognitive ${input.action}`,
    payload: { observedAt: finishedAt, action: input.action, result: finalResult, envelope, relational, executedAgents: cycle.executedAgents, agentInsights, provider: synthesis.ok ? synthesis.provider : null, model: synthesis.ok ? synthesis.model : null, llmStatus: synthesis.ok ? 'COMPLETE' : 'UNAVAILABLE', warnings: synthesis.warnings, hypothesisId },
  }).select('id').single();
  ...
  const twinRun = await registerStudioTwinRun({ taskId: context.taskId!, role: `studio_${input.action}`, objective: `Analyze Studio object ${input.objectId} under active field and SFI methods.`, provider: synthesis.ok ? synthesis.provider : null, model: synthesis.ok ? synthesis.model : null, status: synthesis.ok ? (input.action === 'verify' ? 'VERIFYING' : finalResult.production.status === 'READY' ? 'READY' : 'EVIDENCE_PENDING') : 'BLOCKED', inputSnapshot: { sessionId: studio.session.id, objectId: input.objectId, action: input.action, agentIds: cycle.executedAgents }, outputEnvelope: envelope as unknown as Record<string, unknown>, evidenceRefs: runRefs, limitations: unique([...relational.warnings, ...synthesis.warnings]), startedAt, finishedAt });
  await db.from('studio_archive_events').insert({ session_id: studio.session.id, object_id: input.objectId, owner_id: input.ownerId, event_type: input.action === 'verify' ? 'VERIFICATION_COMPLETED' : input.action === 'generate_hypothesis' ? 'HYPOTHESIS_CREATED' : 'COGNITIVE_ANALYSIS_COMPLETED', label: finalResult.summary ?? `Studio ${input.action}`, source: 'studio_cognitive_runtime_v1', payload: { evidenceId: cognitiveEvidenceId, twinRunId: twinRun.id, hypothesisId, production: finalResult.production, identity: finalResult.identity, ejector: finalResult.ejector } });

  return { ok: true as const, status: 201, objectId: input.objectId, sessionId: studio.session.id, action: input.action, result: finalResult, relational, agents: { executed: cycle.executedAgents, insights: agentInsights }, llm: { ok: synthesis.ok, provider: synthesis.ok ? synthesis.provider : null, model: synthesis.ok ? synthesis.model : null, warnings: synthesis.warnings }, twin: { runId: twinRun.id, evidenceId: cognitiveEvidenceId, contractVersion: twin.contractVersion }, hypothesisId };
}
