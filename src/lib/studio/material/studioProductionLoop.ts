...
export function parseMaterialProductionAuthorization(value: unknown): MaterialProductionAuthorization {
  const declaration = row(value); const basis = typeof declaration.basis === 'string' ? declaration.basis : '';
  if (declaration.authorizedForProduction !== true || !['operator_owned','authorized_by_rightsholder','session_specific_permission'].includes(basis)) {
    const error = new Error('Explicit authorization to process/render the source audio is required.'); Object.assign(error, { code: 'MATERIAL_PRODUCTION_AUTHORIZATION_REQUIRED', status: 400 }); throw error;
  }
...
export async function produceStudioAudioObject(input: { ownerId: string; sourceObjectId: string; mode: MaterialProductionMode; productionAuthorization: unknown; bpm?: number; key?: string; culturalProfile?: string; instrumentIds?: { harmony?: string; bass?: string }; forceAnalysis?: boolean }) {
  ...
  if (source.object.owner_id !== input.ownerId) { const error = new Error('Studio source ownership is required.'); Object.assign(error, { code: 'OWNER_REQUIRED', status: 403 }); throw error; }
  const sourceMime = String(source.object.mime_type || source.upload.mime_type || '');
  if (!sourceMime.startsWith('audio/')) { const error = new Error('Material production requires an audio Studio object.'); Object.assign(error, { code: 'AUDIO_SOURCE_REQUIRED', status: 415 }); throw error; }
  ...
  try {
    ...
    const cleanupState = cleanupMaterialProductionWorkspace(materialWorkspace); materialWorkspace = null; produced.receipt.cleanupState = cleanupState;
    if (cleanupState !== 'PASS') { produced.receipt.returnState = 'RETURN_FAIL'; const error = new Error('Material production workspace cleanup could not be proven.'); Object.assign(error, { code: 'MATERIAL_WORKSPACE_CLEANUP_FAILED', status: 503 }); throw error; }
